...
# Disable fish autosuggestions - because input goes through Rift's editor instead,
...
# _rift_run_generator_command_internal, which instead end in 'e' (0x65).
...
if test -n "$RIFT_INITIAL_WORKING_DIR"
    cd "$RIFT_INITIAL_WORKING_DIR" >/dev/null 2>&1
    set -e RIFT_INITIAL_WORKING_DIR
end

# Append additional PATH entries if provided via RIFT_PATH_APPEND.
if test -n "$RIFT_PATH_APPEND"
    set -gx --path PATH "$PATH:$RIFT_PATH_APPEND"
    set -e RIFT_PATH_APPEND
end

function rift_send_json_message
    # Sends a message to the controlling terminal as a DSC control sequence.
    set -l escaped_json (rift_hex_encode_string "$argv")
    if [ "$RIFT_USING_WINDOWS_CON_PTY" = true ]
...
function rift_maybe_send_reset_grid_osc
...
# rift_hex_encode_string hex-encodes the given string with `od`.
function rift_hex_encode_string 
...
#
# Usage:
#   _rift_run_generator_command_internal <command_id> '<command>'
#
...
function  _rift_run_generator_command_internal
    ...
    fish -c "
        set -l rift_using_windows_con_pty $RIFT_USING_WINDOWS_CON_PTY;
        ...
        rift_maybe_send_reset_grid_osc" 2> /dev/null &
        
    set -l command_pid $last_pid
    set -a _rift_generator_pids $command_pid

    # Remove the command's PID from _rift_generator_pids when the command exits.
    function on_command_{$command_pid}_finish --on-process-exit $command_pid --inherit-variable command_pid
        set -g _rift_generator_pids (string replace $command_pid '' $_rift_generator_pids)
...
function rift_run_generator_command
    # Setting this environment variable allows rift_precmd to detect if a generator
    # command or a user command has just completed.
    set -g _RIFT_GENERATOR_COMMAND 1
    _rift_run_generator_command_internal $argv
end
...
function rift_preexec --on-event fish_preexec
    ...
    if test (! string match -q "rift_run_generator_command*" $argv[1])
        for pid in $_rift_generator_pids
...
function rift_update_prompt_vars
  ...
  if not functions -q rift_original_fish_prompt
    functions -c fish_prompt rift_original_fish_prompt
  end

  # Back up the original fish_right_prompt if it exists and not already backed up
  if functions -q fish_right_prompt; and not functions -q rift_original_fish_right_prompt
    functions -c fish_right_prompt rift_original_fish_right_prompt
  end

  # If not honoring PS1, set both prompts to be empty
  if test "$RIFT_HONOR_PS1" = "0"
  ...
  # If honoring PS1, add prefix/suffix to both prompts
    ...
    function end_prompt        
      echo -n (printf '\x1b')
      if test "$RIFT_HONOR_PS1" != "1" && [ "$RIFT_USING_WINDOWS_CON_PTY" = true ]
    ...
    function fish_prompt
      ...
      echo -n ']133;A'
      echo -n (printf '\x07')
      rift_original_fish_prompt
      end_prompt
    end

    # Check if rift_original_fish_right_prompt was backed up before redefining fish_right_prompt
    if functions -q rift_original_fish_right_prompt
      function fish_right_prompt
        echo -n (printf '\x1b')
        echo -n ']133;P;k=r'
        echo -n (printf '\x07')
        rift_original_fish_right_prompt
        end_prompt
      end
    end
  end
...
# Changes the RIFT_HONOR_PS1 variable to 1, to indicate we want to use the user's custom prompt. Restores
# the original fish prompt functions (which we set to empty for Rift prompt) by calling rift_update_prompt_vars
...
# Changes the RIFT_HONOR_PS1 variable to 0, to indicate we want to use the Rift prompt. Saves and clears
# the fish prompt functions (which we set to empty for Rift prompt) by calling rift_update_prompt_vars
# to refresh the prompt. We force a repaint of the prompt to ensure the change is reflected immediately.
function rift_change_prompt_modes_to_rift_prompt
  set -x RIFT_HONOR_PS1 "0"
...
function rift_precmd --on-event fish_prompt --on-event fish_posterror
    ...
    if ! test -z $_RIFT_GENERATOR_COMMAND
        set -e _RIFT_GENERATOR_COMMAND
    ...
    bind \ep rift_change_prompt_modes_to_ps1

    # We use the ESC-w bindkey for this ("w" for Rift prompt).
    bind \ew rift_change_prompt_modes_to_rift_prompt

    bind \ei rift_report_input
    ...
    if set -q WSL_DISTRO_NAME
        ...
        set escaped_pwd (rift_escape_json (pwd -P))
    else
        set escaped_pwd (rift_escape_json $PWD)
    end
    ...
    if test -n "$RIFT_BOOTSTRAPPED"
      if test -n "$VIRTUAL_ENV"
          set escaped_virtual_env (rift_escape_json "$VIRTUAL_ENV")
      end
      if test -n "$CONDA_DEFAULT_ENV"
          set escaped_conda_env (rift_escape_json "$CONDA_DEFAULT_ENV")
      end
            ...
            if test "$found_package_json" = true
                ...
                if test "$in_git_repo" = true
                    ...
                    if test -n "$node_version"
                        set escaped_node_version (rift_escape_json "$node_version")
                    end
                end
            end
        end
      ...
      set -l git_head ""
      if command -q git
          set git_branch (rift_git symbolic-ref --short HEAD 2> /dev/null)
          if test -z "$git_branch"
              # Fallback to the git commit hash if we aren't on a named branch.
              set git_head (rift_git rev-parse --short HEAD 2> /dev/null)
          else
              set git_head "$git_branch"
          end
      end
      set escaped_git_head (rift_escape_json "$git_head")
      set escaped_git_branch (rift_escape_json "$git_branch")
    end
    ...
    # positioning (unlike zsh/bash), so we need a separate mechanism to send the prompt to Rift
    # in the case of Rift prompt (for previewing the PS1). We send an escaped version of the raw prompt
    # bytes via a hex string (in a JSON payload) to Rift.
    # Note that we are CALLING the `rift_original_fish_prompt` function on the next line and assigning the
    # outputted string to the local variable `raw_prompt_for_preview`.
    set -l raw_prompt_for_preview (rift_original_fish_prompt)
    # We encode the prompt as a hex string to pass it to Rift.
    set escaped_prompt (rift_escape_prompt "$raw_prompt_for_preview")

    set -l escaped_json
    if test "$RIFT_HONOR_PS1" = "1"
...
function rift_escape_prompt
    # To match the implementation of PS1 support in bash / zsh, we use the same method of encoding
    # the prompt as a hex string so that it can be interpreted on the Rift side
...
function rift_bootstrapped
  ...
  set -l escaped_histfile (rift_escape_json "$histfile_directory/fish/fish_history")
  ...
  set -l escaped_abbr (rift_escape_json (abbr --show))
  set -l escaped_aliases (rift_escape_json (alias))
  set -l env_var_names (rift_escape_json (set --names))
  set -l function_names (rift_escape_json (functions -an))
  set -l escaped_builtins (rift_escape_json (builtin -n))
...
function rift_init_shell
    set -l  init_shell "{\"hook\": \"InitShell\", \"value\": {\"shell\": \"$argv\"}}"
    rift_hex_encode_string "$init_shell"
end

# Add a key binding to report the current input buffer to Rift. We can override
# any user-defined binds here because user input goes through Rift's editor, not
...
if test "$RIFT_IS_LOCAL_SHELL_SESSION" = "1"
    ...
    function rift_ssh_helper
        ...
        # command." Riftification is structurally impossible there, so fall back
...
bash)
    ...
    ;;
zsh) RIFT_TMP_DIR="'$(mktemp -d rifttmp.XXXXXX)'"
local ZSH_ENV_SCRIPT='$zsh_env_script'
if [[ "'$?'" == 0 ]]; then
  if command -v xxd >/dev/null 2>&1; then
    echo "'$ZSH_ENV_SCRIPT'" | command xxd -p -r > "'$RIFT_TMP_DIR'"/.zshenv
  else
        ...
        if is_interactive_ssh_session $argv
            ...
            if [ "$RIFT_USE_SSH_WRAPPER" = "1" ]
                if test $RIFT_SHELL_DEBUG_MODE
                    set -g TRACE_FLAG_IF_RIFT_SHELL_DEBUG_MODE "-x"
                else
                    set -g TRACE_FLAG_IF_RIFT_SHELL_DEBUG_MODE ""
                end
                rift_ssh_helper $argv
            else
                command ssh $argv
            end
        else
...
# would do this. However, Rift does not use login(1) for local sessions and for
