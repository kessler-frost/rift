...
if ($PSEdition -eq 'Desktop' -or $IsWindows) {
    ...
    if ((Get-ExecutionPolicy -Scope MachinePolicy) -eq $EP::Restricted -or (Get-ExecutionPolicy -Scope UserPolicy) -eq $EP::Restricted) {
        Write-Error 'ExecutionPolicy is Restricted. Unable to Riftify this PowerShell session.'
        ...
        $global:_rift_PSProcessExecPolicy = $(Get-ExecutionPolicy -Scope Process)
...
function prompt {
    ...
    $function:global:prompt = $global:_riftOriginalPrompt
    $username = [Environment]::UserName
    $global:_riftSessionId = [uint64]@@RIFT_SESSION_ID@@
    $msg = ConvertTo-Json -Compress -InputObject @{ hook = 'InitShell'; value = @{ session_id = $_riftSessionId; shell = 'pwsh'; user = $username; hostname = [System.Net.Dns]::GetHostName() } }
