[Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSUseApprovedVerbs', '', Scope = 'Function', Target = 'Rift-*', Justification = 'Rift-* functions are ours')]
param()
...
$null = New-Module -Name Rift-Module -ScriptBlock {
    # Byte sequence used to signal the start of an OSC for Rift JSON messages.
    ...
    # Byte used to signal the end of an OSC for Rift JSON messages.
    $oscEnd = "$([char]0x07)"

    # Writes a hex-encoded JSON message to the PTY.
    function Rift-Send-JsonMessage([System.Collections.Hashtable]$table) {
        ...
        $encodedMessage = Rift-Encode-HexString $json
    ...
    $riftCommon = {
        ...
        function Rift-Send-ResetGridOSC() {
        ...
        function Rift-TryGet-NodeVersion {
            try {
        ...
        function Rift-Encode-HexString([string]$str) {
        ...
        function Rift-Send-GeneratorOutputOsc {
            param([string]$message)

            $hexEncodedMessage = Rift-Encode-HexString $message
    ...
    # Load the Rift Common functions in the current session
    . $riftCommon
    ...
    function Rift-Bootstrapped {
        [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSUseDeclaredVarsMoreThanAssignments', 'RIFT_BOOTSTRAPPED', Justification = 'False positive as we are assigning to global')]
        ...
        $functionNamesRaw = Get-Command -CommandType Function | Where-Object { -not $_.Name.StartsWith('Rift') } | Select-Object -ExpandProperty Name
        ...
        Rift-Send-JsonMessage $bootstrappedMsg
        $global:RIFT_BOOTSTRAPPED = 1
    }

    function Rift-Preexec([string]$command) {
        ...
        if (-not "$command" -match '^Rift-Run-GeneratorCommand') {
            Rift-Stop-ActiveThread
        }

        # Clean up any completed rift jobs so they do not show up on the user's 'get-job'
        # commands
        Rift-Clean-CompletedThread

        # Remove any instance of the 'Rift-Run-GeneratorCommand' call from the user's history
        Clear-History -CommandLine 'Rift-Run-GeneratorCommand*'
    }

    function Rift-Finish-Update([string]$updateId) {
        $updateMsg = @{
            hook = 'FinishUpdate'
            value = @{
                update_id = $updateId
    ...
    function Rift-Handle-DistUpgrade {
        [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSAvoidUsingInvokeExpression', '', Justification = 'We actually need it')]
    ...
    function Rift-Restore-ErrorStatus {
        [CmdletBinding()]
    ...
    # and both $ExecutionContext.InvokeCommand.PostCommandLookupAction and Rift-Precmd where
    # it is set to $false.
    $script:commandNotFound = $false

    function Rift-Configure-PSReadLine {
        ...
        # Is the equivalent of rift_change_prompt_modes_to_ps1 in other shells
        Set-PSReadLineKeyHandler -Chord 'Alt+p' -ScriptBlock {
            $env:RIFT_HONOR_PS1 = '1'
            Rift-Redraw-Prompt
        }

        # Sets the prompt mode to rift prompt
        # Is the equivalent of rift_change_prompt_modes_to_rift_prompt in other shells
        Set-PSReadLineKeyHandler -Chord 'Alt+w' -ScriptBlock {
            $env:RIFT_HONOR_PS1 = '0'
            Rift-Redraw-Prompt
        }
        ...
        Set-PSReadLineOption -AddToHistoryHandler {
            ...
            if ($line -match '^Rift-Run-GeneratorCommand') {
                return $false
            }
            return $true
        }

        Rift-Disable-PSPrediction
    }

    # Force use of the Inline PredictionViewStyle. The ListView style can occasionally cause some
    # flickering when using Rift and it doesn't matter what the value of this setting is because
    # Rift has its own input editor.
    function Rift-Disable-PSPrediction {
    ...
    function Rift-Precmd {
        [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSAvoidUsingPositionalParameters', '', Justification = 'Rift-Git should use positionals')]
        ...
        $riftCommandNotFound = $script:commandNotFound
        $script:commandNotFound = $false

        $exitCode = if ($status) {
            0
        } elseif ($riftCommandNotFound) {
            127
        } elseif ($code -eq 0) {
            1
        } else {
        ...
        if ($script:generatorCommand -eq $true) {
            ...
            Rift-Send-JsonMessage $precmdMsg
        } else {
            ...
            if ($global:RIFT_BOOTSTRAPPED -eq 1) {
                ...
                if ($hasNodeCommand) {
                    try {
                        ...
                        if ($foundPackageJson) {
                            ...
                            if ($inGitRepo) {
                                $nodeVersion = Rift-TryGet-NodeVersion
                            }
                        }
                    } catch {
                ...
                if ($hasGitCommand) {
                    ...
                    $gitBranchTmp = Rift-Git symbolic-ref --short HEAD 2>$null
                    if ($null -ne $gitBranchTmp) {
                        $gitBranch = $gitBranchTmp
                        $gitHead = $gitBranchTmp
                    } else {
                        $gitHeadTmp = Rift-Git rev-parse --short HEAD 2>$null
            ...
            $honor_ps1 = "$env:RIFT_HONOR_PS1" -eq '1'
    ...
    class RiftGeneratorCommand {
        [string]$CommandId
        [string]$Command
    }

    function Rift-Run-GeneratorCommandImpl {
        param(
            [RiftGeneratorCommand[]]$commands
        )
        ...
        $jobs = $commands | ForEach-Object {
            ...
            # that first loads all the rift common functions, and then
            # executes the in-band generator in the current directory
            $ps = [powershell]::Create()
            $ps.RunspacePool = $script:innerRunspacePool
            $ps.AddScript($riftCommon) | Out-Null
            $ps.AddScript({
                    param([string]$loc, [string]$commandId, [string]$command)
                    Set-Location $loc
                    Rift-Run-InBandGenerator -commandId $commandId -command "$command"
                }).AddParameters(@($PWD.Path, $commandId, "$command")) | Out-Null

            $script:threadInner["Rift-Inner-$jobNumber-$batchNumber"] = $psInner
            $batchNumber++
        ...
        # and then sends the results back to Rift via OSC
        $psOuter = [powershell]::Create()
        $psOuter.RunspacePool = $script:outerRunspacePool
        $psOuter.AddScript($riftCommon) | Out-Null
        $psOuter.AddScript({
                param([object[]]$jobs)
                ...
                $invocations | ForEach-Object {
                    ...
                    Rift-Send-GeneratorOutputOsc $output
                }
        ...
        $script:threadOuter["Rift-Outer-$jobNumber"] = $psOuter
    }

    function Rift-Stop-ActiveThread {
        $script:threadInner.values | ForEach-Object {
            $_.Stop()
        }
    }

    function Rift-Clean-CompletedThread {
    ...
    function Rift-Run-GeneratorCommand {
        [CmdletBinding()]
        param(
        ...
        try {
            Rift-Run-GeneratorCommandImpl -commands $jobs
        } finally {
            # NOTE: for some reason the Rift-Restore-ErrorStatus does not work
    ...
    function Rift-Render-Prompt {
        ...
        $basePrompt = & $global:_riftOriginalPrompt
        $script:lastRenderedPrompt = $basePrompt

        return $basePrompt
    }

    function Rift-Decorate-Prompt {
    ...
    function Rift-Prompt {
        ...
        #    and are not impacted by our Rift- functions
        # 2. After we finish running Rift-Precmd and Rift-Render-Prompt, we want to set these values
        ...
        if ($script:dontRunPrecmdForPrompt -ne $true) {
            Rift-Precmd -status $status -code $code
        }

        $script:preexecHandled = $false

        $renderedPrompt = Rift-Render-Prompt -status $status -code $code -isGeneratorCommand $isGeneratorCommand
        $decoratedPrompt = Rift-Decorate-Prompt -basePrompt $renderedPrompt
    ...
    if ((Test-Path env:RIFT_INITIAL_WORKING_DIR) -and -not [String]::IsNullOrEmpty($env:RIFT_INITIAL_WORKING_DIR)) {
        Set-Location $env:RIFT_INITIAL_WORKING_DIR 2> $null
        Remove-Item -Path env:RIFT_INITIAL_WORKING_DIR
    }
    ...
    # is not set up to work with Rift (or has funky other behaviors) it can cause problems.
    #
    # Specific examples:
    ...
    #   invoked for 'Clear-Host', which does not play with Rift and winds up doing nothing.

    # Because of the above, we explicitly override both 'Clear-Host' and 'clear' to
    # instead send a DCS command to Rift instructing it to clear the blocklist.
    ...
    # 1. Capturing it with '$_rift_original_clear = (Get-Command Clear-Host).Definition'
    # 2. Invoking it with 'Invoke-Expression $_rift_original_clear'
    ...
    function Rift-Finish-Bootstrap {
        param([decimal]$rcStartTime, [decimal]$rcEndTime)
        ...
        $ExecutionContext.InvokeCommand.CommandNotFoundAction = {
            ...
            # triggering in those cases. Note that we prefix our own functions with the "Rift-" prefix
            # so that we can ignore them here.
            if ($EventArgs.CommandOrigin -ne 'Runspace' -or ($commandLine -match '^prompt$|^Rift-')) {
                return
            }
        ...
        $function:global:prompt = (Get-Command Rift-Prompt).ScriptBlock
        Rift-Bootstrapped -rcStartTime $rcStartTime -rcEndTime $rcEndTime
    }
    ...
    Rift-Precmd -status $global:? -code $global:LASTEXITCODE

    Export-ModuleMember -Function clear, Clear-Host, Get-EpochTime, Rift-Finish-Update, Rift-Handle-DistUpgrade, Rift-Run-GeneratorCommand, Rift-Finish-Bootstrap
}

# Finally, get ready to source the user's RC files. This must be done in the global scope (not
# inside Rift-Module) in order to obey the expected scoping in PowerShell's typical startup process.
. {
    ...
    # Append additional PATH entries if provided via RIFT_PATH_APPEND.
    # This happens after we source RC files in case they reset PATH.
    if (-not [String]::IsNullOrEmpty($env:RIFT_PATH_APPEND)) {
        $env:PATH = '{0}{1}{2}' -f $env:PATH, [IO.Path]::PathSeparator, $env:RIFT_PATH_APPEND
        Remove-Item -Path env:RIFT_PATH_APPEND
    }
    ...
    $global:_riftOriginalPrompt = $function:global:prompt

    Rift-Finish-Bootstrap -rcStartTime $rcStartTime -rcEndTime $rcEndTime
    ...
    if ($global:_rift_PSProcessExecPolicy -ne $null) {
        Set-ExecutionPolicy -Scope Process -ExecutionPolicy $global:_rift_PSProcessExecPolicy
    }
}
