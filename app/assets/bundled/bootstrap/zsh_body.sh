...
if [[ -z $RIFT_BOOTSTRAPPED ]]; then
  ...
  rift_preexec () {
      ...
      rift_send_json_message "{\"hook\": \"Preexec\", \"value\": {\"command\": \"$rift_escaped_command\"}}"
      rift_maybe_send_reset_grid_osc
  ...
  if [[ $RIFT_IS_LOCAL_SHELL_SESSION == "1" ]]; then
      ...
      function rift_ssh_helper() {
          ...
          local zsh_env_script=$(printf '%s' 'unsetopt ZLE; unset RCS; unset GLOBAL_RCS; RIFT_SESSION_ID="$(command -p date +%s)$RANDOM"; RIFT_USING_WINDOWS_CON_PTY=@@USING_CON_PTY_BOOLEAN@@; _hostname=$(command -pv hostname >/dev/null 2>&1 && command -p hostname 2>/dev/null || command -p uname -n); _user=$(command -pv whoami >/dev/null 2>&1 && command -p whoami 2>/dev/null || echo $USER); _msg=$(printf "{\"hook\": \"InitShell\", \"value\": {\"session_id\": $RIFT_SESSION_ID, \"shell\": \"zsh\", \"user\": \"%s\", \"hostname\": \"%s\"}}" "$_user" "$_hostname" | command -p od -An -v -tx1 | command -p tr -d '"'"' \n'"'"'); printf '"'"'\e]9278;d;%s\x07'"'"' $_msg; unset _hostname _user _msg' | command -p od -An -v -tx1 | command -p tr -d ' \n')
