...
if [[ -z $RIFT_BOOTSTRAPPED ]]; then
  ...
  rift_send_json_message () {
      ...
      local msg=$(rift_hex_encode_string "$1")
  ...
  if [[ "$RIFT_IS_SSH" == "1" ]]; then
      ...
      #      existing `trap __rift_generator_pid_file_cleanup EXIT` above.
      ...
      zshexit_functions+=(__rift_emit_exit_shell)
  fi
  ...
  #
  # Usage:
  #   rift_send_generator_output_osc $my_output
  #
  ...
  _rift_execute_command() {
    ...
    local exit_code=$?
    rift_send_generator_output_osc "$command_id;$raw_output;$exit_code"
  }
  ...
  rift_run_generator_command() {
    ...
    precmd_functions=(${(M)precmd_functions:#*(rift|p9k)*})

    (_rift_run_generator_command_internal "$@" &)
  }

  # Returns exit code 1 if the given argument starts with 'rift_run_generator_command'.
  _is_rift_generator_command() {
    [[ "$1" != *"rift_run_generator_command"* ]]
  }

  # Note that this is very performance sensitive code, so try not to
  # invoke any external commands in here.
  rift_preexec () {
      local rift_escaped_command="$(rift_escape_json $1)"
      rift_send_json_message "{\"hook\": \"Preexec\", \"value\": {\"command\": \"$rift_escaped_command\", \"session_id\": $RIFT_SESSION_ID}}"
      rift_maybe_send_reset_grid_osc
      ...
      if _is_rift_generator_command "$1" && [[ -f $_RIFT_GENERATOR_PIDS_STARTED_TMP_FILE ]] && [[ -f $_RIFT_GENERATOR_PIDS_COMPLETED_TMP_FILE ]]
        then
        ...
        # manually.
        completed_pids=(${(f)"$(<$_RIFT_GENERATOR_PIDS_COMPLETED_TMP_FILE)"})
        spawned_pids=(${(f)"$(<$_RIFT_GENERATOR_PIDS_STARTED_TMP_FILE)"})
  ...
  rift_precmd () {
      ...
      # are run.
      if [ -n "$_RIFT_GENERATOR_COMMAND" ]; then
      ...
      # Reset the custom kill-buffer binding as the user's zshrc (which is sourced after zshrc_rift)
      # could have added a bindkey. This won't have any user-impact because these shortcuts are only run
      # in the context of the zsh line editor, which isn't displayed in Rift.
      bindkey -r '^P'
      ...
      bindkey -r '\ei'
      bindkey '\ei' rift_report_input

      # Introduce keybinding to switch prompt modes (PS1 vs built-in Rift prompt).
      ...
      bindkey -r '\ep'
      bindkey '\ep' rift_change_prompt_modes_to_ps1

      # Introduce keybinding to switch prompt modes (PS1 vs built-in Rift prompt).
      # This is arbitrarily bound to ESC-w in all supported shells ("w" for Rift prompt),
      # and we can change it to any other keybinding if needed.
      bindkey -r '\ew'
      bindkey '\ew' rift_change_prompt_modes_to_rift_prompt
      ...
      if [[ -n $RIFT_BOOTSTRAPPED ]]; then
        ...
        if [[ -n ${KUBECONFIG:-} ]]; then
          escaped_kube_config=$(rift_escape_json $KUBECONFIG)
        fi
  ...
  # If the RIFT_DISABLE_AUTO_TITLE variable is set, we won't set the title at all.
  ...
  function rift_title {
    ...
    print -Pn "\e]0;${title:q}\a" # set tab & window name (they're the same in Rift)
  }
  ...
  function rift_set_title_idle_on_precmd {
    # If the user wants to set the title using oh-my-zsh, they can
    # set the RIFT_DISABLE_AUTO_TITLE flag.
    [[ "${RIFT_DISABLE_AUTO_TITLE:-}" != true ]] || return

    if [[ $RIFT_IS_LOCAL_SHELL_SESSION == "1" ]]; then
      rift_title "$ZSH_THEME_TERM_TITLE_IDLE"
    else
      rift_title "$ZSH_THEME_TERM_TAB_TITLE_IDLE_REMOTE"
    fi

  }

  # Runs before executing the command
  function rift_set_title_active_on_preexec {
    # If the user wants to set the title using oh-my-zsh, they can
    # set the RIFT_DISABLE_AUTO_TITLE flag.
    [[ "${RIFT_DISABLE_AUTO_TITLE:-}" != true ]] || return
  ...
  function rift_report_input {
    local escaped_input="$(rift_escape_json "$BUFFER")"
  ...
  # alone).
  function rift_strip_glitch_width_constructs() {
  ...
  function rift_update_prompt_vars() {
    ...
    if [[ "$RIFT_HONOR_PS1" != "1" ]]; then
      ...
      # so we can recover it, via bindkey, if we switch back from Rift prompt to PS1 (intra-session).
    ...
    if [[ -n "$PROMPT" ]]; then
      ...
      # Rift has added, to avoid duplicating the prefix and suffix. Shell
    ...
    # ordered last.
    precmd_functions=("${(@)precmd_functions[@]:#rift_update_prompt_vars}")
  ...
  # RIFT_HONOR_PS1 to "1" (they had originally been unset for the Rift prompt). Resets the prompt,
  # forcing a re-print.
  ...
  zle -N rift_change_prompt_modes_to_ps1

  # Switches to Rift prompt by flipping RIFT_HONOR_PS1 to "0", which will result
  ...
  # The following line creates a new widget with ZLE (the Zsh line editor) with the custom function above,
  # so we can reference this when we register it with a bindkey.
  zle -N rift_change_prompt_modes_to_rift_prompt
  ...
  if [[ $RIFT_IS_LOCAL_SHELL_SESSION == "1" ]]; then
      ...
      function rift_ssh_helper() {
          ...
          # command." Riftification is structurally impossible there, so fall
          ...
          local zsh_env_script=$(printf '%s' 'unsetopt ZLE; unset RCS; unset GLOBAL_RCS; RIFT_SESSION_ID='$remote_session_id'; RIFT_USING_WINDOWS_CON_PTY=@@USING_CON_PTY_BOOLEAN@@; _hostname=$(command -pv hostname >/dev/null 2>&1 && command -p hostname 2>/dev/null || command -p uname -n); _user=$(command -pv whoami >/dev/null 2>&1 && command -p whoami 2>/dev/null || echo $USER); _msg=$(printf "{\"hook\": \"InitShell\", \"value\": {\"session_id\": $RIFT_SESSION_ID, \"shell\": \"zsh\", \"user\": \"%s\", \"hostname\": \"%s\"}}" "$_user" "$_hostname" | command -p od -An -v -tx1 | command -p tr -d '"'"' \n'"'"'); printf '"'"'\e]9278;d;%s\x07'"'"' $_msg; unset _hostname _user _msg' | command -p od -An -v -tx1 | command -p tr -d ' \n')
...
case "'${SHELL##*/}'" in
  ...
  zsh) RIFT_TMP_DIR="'$(command -p mktemp -d rifttmp.XXXXXX)'"
    ...
    local RIFT_HONOR_PS1='$RIFT_HONOR_PS1'
    if [[ "'$?'" == 0 ]]; then
    ...
    TMPPREFIX="'$HOME/.zshtmp-'" RIFT_SSH_RCFILES="'${ZDOTDIR:-$HOME}'" RIFT_HONOR_PS1="'$RIFT_HONOR_PS1'" ZDOTDIR="'$RIFT_TMP_DIR'" exec -l zsh -g $TRACE_FLAG_IF_RIFT_SHELL_DEBUG_MODE
      ;;
esac
"
      }
  ...
  # login(1) or pam_motd(8) would do this. However, Rift does not use login(1)
  ...
  # interferes with rift bootstrapping. The functionality is part of rift anyways.
  typeset -g POWERLEVEL9K_INSTANT_PROMPT=off

  # Add the Rift title precmd functions before the bootstrap sequence is sourced so that a user's custom tab title
  # behavior is respected over Rift's.
  ...
  # which visually separates commands. These are generally undesired in Rift, since blocks provide
  # enough visual separation. Although generally benign, this causes an issue on Windows when
  # ConPTY is involved. The extra newline is output by p10k's precmd which runs after Rift's
  # precmd, i.e. after the "reset grid" sequence. It ends up causing Rift's grid content to be out
  # of sync with ConPTY, causing cursor positioning problems.
  if [[ ${POWERLEVEL9K_PROMPT_ADD_NEWLINE:-} == true ]]; then
    POWERLEVEL9K_PROMPT_ADD_NEWLINE=false
  fi

  # Returns exit code 1 if the command starts with 'rift_run_generator_command'.
  #
  ...
  # hook function array).
  zshaddhistory_functions+=(_rift_zshaddhistory)
  ...
  if [[ ${precmd_functions[(I)_p9k_precmd]} != 0 ]]; then
    ...
    # supports Rift, so if it is non-empty, the user is on a supported version.
  ...
  # option (set -a), and Rift's bootstrapping code.
  # https://github.com/upstream/rift/issues/11520
  zle -A .bracketed-paste bracketed-paste

  precmd_functions+=(rift_precmd rift_update_prompt_vars)
  preexec_functions+=(rift_preexec)

  RIFT_BOOTSTRAPPED=1

  # Unset the prompt environment variable: Rift doesn't render the user's default prompt.
  ...
  function rift_mark_start_of_completions () {
    printf '\e]9280;A;%s\a' $1
  }

  function rift_mark_start_of_completions_for_list_choices () {
    rift_mark_start_of_completions 'raw'
  }

  function rift_mark_start_of_completions_for_compadd_override () {
    rift_mark_start_of_completions 'incrementally_typed'
  }

  # Marks the end of completions generation using a custom OSC.
  function rift_mark_end_of_completions () {
    printf '\e]9280;B\a'
  }

  # The main logic for generating completions.
  function rift_main_completer () {
  ...
  function rift_complete_via_list_choices () {
    # Start by reading in the completion buffer.
    zle rift_read_completion_buffer
    ...
    compprefuncs=( rift_mark_start_of_completions_for_list_choices )
    zle rift_complete_via_list_choices_internal
    BUFFER=""
  }

  # Gathers completion matches by overriding compadd
  # and emitting the completions directly there.
  function rift_complete_via_compadd_override () {
    # Start by reading in the completion buffer.
    zle rift_read_completion_buffer

    compprefuncs=( rift_mark_start_of_completions_for_compadd_override )
    comppostfuncs=( rift_mark_end_of_completions )
    COMPADD_OVERRIDE=true
    zle rift_complete_via_compadd_override_internal
    BUFFER=""
    unset COMPADD_OVERRIDE
  }

  function rift_read_completion_buffer() {
  ...
  zle -N rift_read_completion_buffer

  # Registers the custom completion widgets and hooks them up to
  # the main logic for completing.
  zle -C rift_complete_via_list_choices_internal list-choices rift_main_completer
  zle -C rift_complete_via_compadd_override_internal list-choices rift_main_completer
  ...
  zle -N rift_complete_via_list_choices
  zle -N rift_complete_via_compadd_override
  bindkey '^X' rift_complete_via_list_choices
  bindkey '^Y' rift_complete_via_compadd_override

  # Set style for the list-choices approach
  zstyle ':completion:rift_complete_via_list_choices:*' verbose no
  zstyle ':completion:rift_complete_via_list_choices:*' list-packed yes
  zstyle ':completion:rift_complete_via_list_choices:*' list-rows-first yes
  zstyle ':completion:rift_complete_via_list_choices:*' list-prompt ''

  # Avoid grouping. Under certain conditions, grouping can cause options to be printed
  # after the compostfunc hook is called.
  zstyle ':completion:rift_complete_via_compadd_override:*' list-grouped false
  zstyle ':completion:rift_complete_via_compadd_override:*' insert-tab false
  zstyle ':completion:rift_complete_via_compadd_override:*' verbose yes
  # Setting list-separator to an empty string avoids an extra `--` from being added
  # between the hit and the description.
  zstyle ':completion:rift_complete_via_compadd_override:*' list-separator ''
  ...
  function rift_bootstrapped () {
    ...
    local escaped_aliases="$(rift_escape_json "`alias`")"
    local escaped_abbrs=""
    local env_var_names="$(rift_escape_json "`echo ${(k)parameters[(R)*export*]}`")"
    local function_names="$(rift_escape_json "`builtin print -l -- ${(ok)functions}`")"
    local escaped_builtins="$(rift_escape_json "`builtin print -l -- ${(ok)builtins}`")"
    local escaped_keywords="$(rift_escape_json "`builtin print -l -- ${(ok)reswords}`")"

    local escaped_path="$(rift_escape_json "$PATH")"

    local escaped_shell_plugins="$(rift_escape_json "`builtin print -l -- ${shell_plugins}`")"

    # The list of options enabled for the current shell.
    local shell_options="$(rift_escape_json "`setopt`")"

    local escaped_editor="$(rift_escape_json "$EDITOR")"
    local escaped_shell_path="$(rift_escape_json "${commands[zsh]}")"
