...
use riftui::r#async::executor::BackgroundTask;
...
pub enum StartupArgsForwardingError {
    /// There's no instance of Rift already running.
...
impl From<zbus::fdo::Error> for StartupArgsForwardingError {
    fn from(value: zbus::fdo::Error) -> Self {
        ...
        // registering a service, so this really means that Rift is not already
        // running.
...
// A D-Bus client for connecting to an already-running instance of Rift and
...
impl DBusServiceHost {
    fn new(ctx: &mut riftui::ModelContext<Self>) -> Self {
    ...
    fn terminate(&mut self) {
        if let Some(server_task) = self.server_task.take() {
            ...
            report_if_error!(riftui::r#async::block_on(server_task));
        }
    }
...
impl riftui::Entity for DBusServiceHost {
    type Event = ();
}
