//! Generates a JSON Schema file describing Rift's user-facing settings.
//!
//! Usage:
//! ```
...
fn ensure_settings_linked() {
    let _ = std::hint::black_box(rift::settings::RESTORE_SESSION);
}
...
fn main() {
    ...
    root.insert(
        "title".to_string(),
        Value::String("Rift Settings".to_string()),
    );
    root.insert(
        "description".to_string(),
        Value::String(format!(
            "JSON Schema for Rift settings ({channel} channel, {entry_count} settings)"
        )),
    );
