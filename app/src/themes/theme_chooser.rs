...
use riftui::platform::{Cursor, SystemTheme};
...
use crate::user_config::{RiftConfig, RiftConfigUpdateEvent, load_theme_configs, themes_dir};
...
fn theme_chooser_items(theme_config: &RiftThemeConfig) -> Vec<ThemeChooserItem> {
    let mut theme_items: Vec<ThemeChooserItem> = theme_config
        ...
        .filter(|(key, _)| match key {
            // Referral reward themes are cloud-only; never shown in local build
