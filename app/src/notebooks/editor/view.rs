...
use rift_editor::content::text::{BufferTextStyle, CodeBlockType, TextStyles};
use rift_editor::content::version::BufferVersion;
use rift_editor::editor::{EmbeddedItemModel, NavigationKey, RunnableCommandModel, TextDecoration};
...
use rift_editor::render::model::{BlockItem, HitTestBlockType, Location, RenderState};
...
use riftui::assets::asset_cache::{AssetCache, AssetHandle, AssetState};
...
use riftui::fonts::{FallbackFontEvent, FallbackFontModel};
...
pub enum EditorViewAction {
    ...
    InsertBlock(rift_editor::content::text::BlockType),
    Indent,
    Unindent,
    Tab,
    ShiftTab,
...
fn mermaid_diagram_needs_loaded_layout(
    config: &rift_editor::render::model::ImageBlockConfig,
    image: &ImageType,
) -> bool {
...
impl RichTextEditorView {
    ...
    pub(super) fn insert_block(
        &mut self,
        block_type: rift_editor::content::text::BlockType,
        ctx: &mut ViewContext<Self>,
    ) {
        enum InsertionMode {
            ...
            self.model.update(ctx, |model, ctx| {
                match insertion_mode {
                    ...
                    InsertionMode::DeleteSlashAndRestyleLine(cursor_position) => match block_type {
                        rift_editor::content::text::BlockType::Item(item) => {
                        ...
                        rift_editor::content::text::BlockType::Text(style) => {
                            // Note: We can use `set_last_selection_head` because the menu cannot
                            // be opened when there are multiple selections.
...
impl View for RichTextEditorView {
    ...
    fn active_cursor_position(&self, ctx: &ViewContext<Self>) -> Option<riftui::CursorInfo> {
...
impl rift_editor::editor::EditorView for RichTextEditorView {
