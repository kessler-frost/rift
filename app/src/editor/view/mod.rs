...
use rift_core::{safe_error, send_telemetry_from_ctx};
...
pub fn init(ctx: &mut AppContext) {
    ...
    ctx.register_fixed_bindings(vec![
        ...
        // binding also works in Rift. We should not break their workflow.
        FixedBinding::new(
            "ctrl-right",
...
pub type KeymapContextModifierFn = Box<dyn Fn(&mut riftui::keymap::Context, &AppContext)>;
...
impl EditorView {
    ...
    pub fn process_and_attach_images_as_ai_context(
        ...
        self.process_attached_images_future_handle = Some(ctx.spawn(
            async move {
                let mut num_processed_images: usize = 0;
    ...
    fn process_non_image_files(&mut self, _file_paths: Vec<String>, _ctx: &mut ViewContext<Self>) {}
    ...
    pub fn move_to_paragraph_start(&mut self, ctx: &mut ViewContext<Self>) {
        // rift doesn't wrap the text, so basically each line is a paragraph.
    ...
    fn drag_and_drop_files(&mut self, paths: &[UserInput<String>], ctx: &mut ViewContext<Self>) {
        ...
        let image_filepaths =
            riftui::clipboard_utils::get_image_filepaths_from_paths(&paths_as_strings);
        ...
        let input =
            riftui::clipboard_utils::escaped_paths_str(&transformed_paths, self.shell_family);
        ...
        match action {
            ...
            UnhandledModifierKey(keystroke) => {
                if self.can_select(ctx) {
                    ...
                    // try to use in the editor but are currently not available in Rift.
