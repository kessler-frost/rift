...
use riftui::{Entity, SingletonEntity, TypedActionView, View, ViewContext};
...
impl View for AutosuggestionIgnore {
    ...
    fn render(&self, ctx: &riftui::AppContext) -> Box<dyn Element> {
        ...
        let line_height_ratio = appearance
            .line_height_ratio()
            .min(riftui::elements::DEFAULT_UI_LINE_HEIGHT_RATIO);
        ...
        Hoverable::new(self.autosuggestion_ignore_mouse_handle.clone(), |state| {
            ...
            let close_icon = Container::new(
                ConstrainedBox::new(Icon::X.to_riftui_icon(Fill::Solid(icon_color)).finish())
            ...
            let mut ignore_button = Container::new(close_icon)
                ...
                .with_border(
                    riftui::elements::Border::all(border_width).with_border_color(disabled_color),
                )
