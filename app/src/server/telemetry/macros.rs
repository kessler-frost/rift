...
macro_rules! send_telemetry_sync_from_ctx {
    ($event:expr, $ctx:expr) => {
        ...
        if event.enablement_state().is_enabled() {
            let server_api =
                <$crate::server::server_api::ServerApiProvider as riftui::SingletonEntity>::handle(
                    $ctx,
                )
                .as_ref($ctx)
                .get();
            let privacy_settings_snapshot =
                <$crate::settings::PrivacySettings as riftui::SingletonEntity>::handle($ctx)
                    .as_ref($ctx)
                    .get_snapshot($ctx);
            let _ = $ctx.spawn(
                async move {
    ...
    ($event:expr, $app_ctx:expr) => {
        ...
        if $event.enablement_state().is_enabled() {
            let server_api =
                ...
                .get();
            let privacy_settings_snapshot =
                <$crate::settings::PrivacySettings as riftui::SingletonEntity>::handle($app_ctx)
                    .as_ref($app_ctx)
...
macro_rules! send_telemetry_on_executor {
    ($auth_state: expr, $event:expr, $executor:expr) => {
        ...
        if event.enablement_state().is_enabled() {
            ...
            riftui::record_telemetry_on_executor!(
                user_id,
                anonymous_id,
                event.name().into(),
                event.payload(),
