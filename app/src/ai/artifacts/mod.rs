...
impl TryFrom<rift_graphql::ai::AIConversationArtifact> for Artifact {
    type Error = ();

    fn try_from(value: rift_graphql::ai::AIConversationArtifact) -> Result<Self, Self::Error> {
        match value {
            rift_graphql::ai::AIConversationArtifact::PlanArtifact(plan) => Ok(Artifact::Plan {
            ...
            rift_graphql::ai::AIConversationArtifact::PullRequestArtifact(pr) => {
                let (repo, number) = parse_github_pr_url(&pr.url).unzip();
            ...
            rift_graphql::ai::AIConversationArtifact::ScreenshotArtifact(screenshot) => {
            ...
            rift_graphql::ai::AIConversationArtifact::FileArtifact(file) => Ok(Artifact::File {
            ...
            rift_graphql::ai::AIConversationArtifact::Unknown => Err(()),
        }
    }
}
...
pub fn open_screenshot_lightbox<V: riftui::View>(
    artifact_uids: &[String],
    ctx: &mut riftui::ViewContext<V>,
) {
...
pub fn download_file_artifact<V: riftui::View>(
    artifact_uid: &str,
    ctx: &mut riftui::ViewContext<V>,
) {
    let ai_client = ServerApiProvider::handle(ctx).as_ref(ctx).get_ai_client();
...
fn open_file_download_result<V: riftui::View>(
    artifact_uid: &str,
...
#[cfg(feature = "local_fs")]
fn open_file_download_picker<V: riftui::View>(
    artifact: ArtifactDownloadResponse,
    ctx: &mut riftui::ViewContext<V>,
) {
    ...
    ctx.open_save_file_picker(
        move |path_opt: Option<String>, _me: &mut V, ctx: &mut riftui::ViewContext<V>| {
...
fn show_file_download_toast<V: riftui::View>(
    artifact_uid: &str,
