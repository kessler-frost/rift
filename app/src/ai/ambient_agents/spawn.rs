...
fn poll_run_until_joinable_session(
    ...
    async_stream::stream! {
        ...
        let mut timeout_timer = FutureExt::fuse(match timeout {
            Some(d) => riftui::r#async::Timer::after(d),
            None => riftui::r#async::Timer::never(),
        });
        ...
        loop {
            let mut poll_timer = FutureExt::fuse(riftui::r#async::Timer::after(TASK_STATUS_POLL_INTERVAL));
