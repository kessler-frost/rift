...
fn task_id_for_message_send_prefers_sender_run_id() {
    std::env::set_var(rift_cli::OZ_RUN_ID_ENV, OTHER_TASK_ID);
    let task_id = task_id_for_message_send(TASK_ID)
        .expect("valid task id")
        .expect("task id");
    std::env::remove_var(rift_cli::OZ_RUN_ID_ENV);
...
fn task_id_for_message_send_falls_back_to_oz_run_id() {
    std::env::set_var(rift_cli::OZ_RUN_ID_ENV, TASK_ID);
...
fn task_id_from_oz_run_id_env_rejects_invalid_value() {
    std::env::set_var(rift_cli::OZ_RUN_ID_ENV, "not-a-task-id");
