...
use rift_cli::federate::{FederateCommand, IssueGcpTokenArgs, IssueTokenArgs};
...
fn issue_token(
    ...
    ManagedSecretManager::handle(ctx).update(ctx, move |manager, ctx| {
        let future =
            manager.issue_task_identity_token(rift_managed_secrets::client::IdentityTokenOptions {
                audience,
