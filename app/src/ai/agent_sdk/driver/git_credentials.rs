...
async fn try_refresh(task_id: &str, ai_client: &Arc<dyn AIClient>) -> Result<()> {
    let workload_token =
        rift_isolation_platform::issue_workload_token(Some(Duration::from_secs(5 * 60)))
            .await
...
pub(crate) async fn refresh_loop(task_id: String, ai_client: Arc<dyn AIClient>) {
    loop {
        riftui::r#async::Timer::after(GIT_CREDENTIALS_REFRESH_INTERVAL).await;
        ...
        loop {
            match try_refresh(&task_id, &ai_client).await {
                ...
                Err(e) if attempt < backoff_delays.len() => {
                    ...
                    riftui::r#async::Timer::after(delay).await;
                    attempt += 1;
                }
                Err(e) => {
                    log::warn!(
