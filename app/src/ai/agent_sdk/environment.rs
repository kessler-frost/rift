...
const RIFT_DEV_ENVIRONMENTS_REPO: &str = "https://github.com/warpdotdev/warp-dev-environments";
...
impl EnvironmentCommandRunner {
    ...
    fn get(&mut self, id: String, ctx: &mut ModelContext<Self>) {
        ...
        ctx.spawn(initial_sync, move |_, result, ctx| {
            ...
            let server_id = match ServerId::try_from(id.as_str()) {
                ...
                Err(_) => {
                    ctx.terminate_app(
                        riftui::platform::TerminationMode::ForceTerminate,
    ...
    fn create(
        ...
        scope: rift_cli::scope::ObjectScope,
...
impl riftui::Entity for EnvironmentCommandRunner {
    type Event = ();
}
