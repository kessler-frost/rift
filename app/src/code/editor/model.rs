...
use rift_editor::content::edit::EditDelta;
use rift_editor::content::find::{SearchConfig, SearchResults};
use rift_editor::content::hidden_lines_model::HiddenLinesModel;
use rift_editor::content::selection_model::BufferSelectionModel;
use rift_editor::content::text::{BufferBlockStyle, IndentBehavior, IndentUnit};
use rift_editor::content::version::BufferVersion;
use rift_editor::decoration::DecorationLayer;
use rift_editor::editor::TextDecoration;
use rift_editor::model::{CoreEditorModel, PlainTextEditorModel};
use rift_editor::multiline::{AnyMultilineString, MultilineString, LF};
...
use rift_editor::selection::{SelectionMode, SelectionModel, TextDirection, TextUnit};
...
impl CodeEditorModel {
    ...
    pub fn set_hidden_lines(
        &mut self,
        ranges: RangeSet<rift_editor::content::text::LineCount>,
        ctx: &mut ModelContext<Self>,
    ) {
    ...
    pub fn set_visible_line_range(
        &mut self,
        range: Range<rift_editor::content::text::LineCount>,
        ctx: &mut ModelContext<Self>,
    ) {
    ...
    fn calculate_hidden_lines(&mut self, ctx: &mut ModelContext<Self>) {
        if let Some(context_line) = self.hide_lines_outside_of_active_diff {
            ...
            let mut visible_ranges: RangeSet<rift_editor::content::text::LineCount> =
                RangeSet::new();
            ...
            let all_lines: Range<rift_editor::content::text::LineCount> =
                rift_editor::content::text::LineCount::from(0)
                    ..rift_editor::content::text::LineCount::from(line_count);

            // Find gaps in the visible ranges
            let hidden_ranges = visible_ranges
                .gaps(&all_lines)
                .collect::<RangeSet<rift_editor::content::text::LineCount>>();
    ...
    pub fn autocomplete_symbol(&mut self, open: char, close: char, ctx: &mut ModelContext<Self>) {
        ...
        if all_cursors {
            ...
            if let Ok(texts) = vec1::Vec1::try_from_vec(texts) {
                self.update_content(
                    |mut content, ctx| {
                        content.apply_edit(
                        rift_editor::content::buffer::BufferEditAction::InsertForEachSelection {
                            texts: &texts,
                        },
                        rift_editor::content::buffer::EditOrigin::UserTyped,
                        selection_model,
                        ctx,
                    );
                    },
        ...
        if let Ok(edits) = vec1::Vec1::try_from_vec(edits) {
            ...
            self.update_content(
                |mut content, ctx| {
                    content.apply_edit(
                        rift_editor::content::buffer::BufferEditAction::InsertAtCharOffsetRanges {
                            edits: &edits,
                        },
                        rift_editor::content::buffer::EditOrigin::UserTyped,
                        selection_model,
...
impl CoreEditorModel for CodeEditorModel {
    ...
    fn validate(&self, _ctx: &impl riftui::ModelAsRef) {}

    // Since this is a plain text editor, there is no text styles.
    fn active_text_style(&self) -> rift_editor::content::text::TextStyles {
        Default::default()
    }
