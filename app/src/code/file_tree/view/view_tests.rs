...
fn std_path(path: &std::path::Path) -> rift_util::standardized_path::StandardizedPath {
    rift_util::standardized_path::StandardizedPath::try_from_local(path).unwrap()
}
...
fn build_repo_state(repo_root: &std::path::Path) -> FileTreeState {
    ...
    let src_dir = Entry::Directory(DirectoryEntry {
        path: rift_util::standardized_path::StandardizedPath::try_from_local(
...
fn flattened_paths(
    view: &FileTreeView,
    root: &std::path::Path,
) -> Vec<rift_util::standardized_path::StandardizedPath> {
...
fn hidden_files_are_filtered_until_setting_is_enabled() {
    VirtualFS::test("file_tree_hidden_files_setting", |dirs, mut vfs| {
        ...
        App::test((), |mut app| async move {
            ...
            <crate::settings::CodeSettings as riftui::SingletonEntity>::handle(&app).update(
                &mut app,
                |settings, ctx| {
...
fn repo_transition_unregisters_lazy_loaded_path() {
    VirtualFS::test("file_tree_repo_transition", |dirs, mut vfs| {
        ...
        let canonical_repo_root =
            rift_util::standardized_path::StandardizedPath::from_local_canonicalized(&repo_root)
                .unwrap();
        ...
        App::test((), |mut app| async move {
            ...
            file_tree_view.read(&app, |view, _ctx| {
                ...
                let displayed_std =
                    rift_util::standardized_path::StandardizedPath::try_from_local(&displayed_root)
                        .unwrap();
                assert_eq!(
                    view.root_directories
                        .get(&displayed_std)
            ...
            file_tree_view.read(&app, |view, _ctx| {
                let displayed_std =
                    rift_util::standardized_path::StandardizedPath::try_from_local(&displayed_root)
                        .unwrap();
                let repo_std =
                    rift_util::standardized_path::StandardizedPath::try_from_local(&repo_root)
                        .unwrap();
        ...
        App::test((), |mut app| async move {
            ...
            file_tree_view.update(&mut app, |view, ctx| {
                view.ensure_loaded_path(
                    &rift_util::standardized_path::StandardizedPath::try_from_local(&repo_root)
                        .unwrap(),
                    &rift_util::standardized_path::StandardizedPath::try_from_local(&src_dir)
                        .unwrap(),
                    ctx,
                );
            });
            ...
            file_tree_view.update(&mut app, |view, ctx| {
                view.ensure_loaded_path(
                    &rift_util::standardized_path::StandardizedPath::try_from_local(&repo_root)
                        .unwrap(),
                    &rift_util::standardized_path::StandardizedPath::try_from_local(&nested_dir)
                        .unwrap(),
                    ctx,
                );
            });
