...
use riftui::{Entity, GetSingletonModelHandle, ModelContext, SingletonEntity, UpdateModel};
...
pub struct PromptConfiguration {
    ...
    /// The separator to use as a trailing character at the end of Rift prompt, if any.
...
impl PromptSelection {
    ...
    pub fn separator(&self) -> RiftPromptSeparator {
        match self {
            PromptSelection::Default => RiftPromptSeparator::None,
...
impl Prompt {
    ...
    /// Reset to the default Rift prompt.
    ...
    /// Whether same line prompt is enabled for Rift prompt.
    pub fn same_line_prompt_enabled(&self) -> bool {
        self.config.same_line_prompt_enabled
    }

    /// The separator to be used for the Rift prompt.
    pub fn separator(&self) -> RiftPromptSeparator {
...
impl PromptConfiguration {
    /// The default Rift prompt, synthesized from legacy prompt settings.
    ...
    pub fn default_prompt_with_pr_chip_suppressed(suppress_pr_chip: bool) -> Self {
        ...
        Self::from_chips(chips, false, RiftPromptSeparator::None)
    }
