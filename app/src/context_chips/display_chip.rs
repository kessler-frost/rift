...
pub fn render_git_diff_stats_content(
    ...
    let icon_element = if has_changes {
        ...
        Icon::File
            .to_riftui_icon(Fill::Solid(internal_colors::neutral_6(theme)))
            .finish()
    } else {
    ...
    if branch.is_linked_worktree {
        return format!(
            ...
            shell_single_quote(&format!(
                "Branch '{}' is already checked out in another worktree, but Rift couldn't find its path.",
                branch.branch_name
            ))
        );
    }
...
pub(crate) fn render_udi_chip(config: UdiChipConfig, appearance: &Appearance) -> Box<dyn Element> {
    ...
    if let Some(icon) = config.icon {
        content.add_child(
            Container::new(
                ConstrainedBox::new(icon.to_riftui_icon(Fill::Solid(config.color)).finish())
                    .with_height(icon_size)
                    .with_width(icon_size)
                    .finish(),
            )
