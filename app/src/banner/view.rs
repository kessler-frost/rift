...
pub enum DismissalType {
    ...
    /// Dismissal state should also persist across app sessions (e.g. when Rift is restarted).
    Permanent,
}
...
impl<T: Action + Clone> Banner<T> {
    ...
    fn render_icon(appearance: &Appearance, icon: &Icon) -> Box<dyn Element> {
        ...
        ConstrainedBox::new(icon.to_riftui_icon(appearance.theme().accent()).finish())
