...
pub struct SessionContext {
    ...
    /// Snapshot of all Rift workflow aliases.
...
impl GeneratorContext for SessionContext {
    async fn execute_command_at_pwd(
        ...
        // because if the user opened Rift through a parent process that didn't have the PATH var set
...
impl CompletionContext for SessionContext {
    ...
    #[cfg(feature = "completions_v2")]
    fn js_context(&self) -> Option<&dyn rift_completer::completer::JsExecutionContext> {
        self.js_ctx
            .as_ref()
            .map(|ctx| -> &dyn rift_completer::completer::JsExecutionContext { ctx })
    }
...
impl SessionContext {
    pub fn new(
        ...
        // The cloud-synced WorkflowAliases model was removed; no workflow aliases are sourced.
...
fn ls_script_for_dir(directory: &TypedPath) -> Option<String> {
    ...
    let escaped_dir = rift_util::path::ShellFamily::Posix.shell_escape(dir_str);
