//! Login-item registration — makes Rift start automatically when the user
...
/// Reconciles whether Rift is registered to launch at login with the user's
/// current preference.
///
/// Respects the existing `app_added_as_login_item` bookkeeping so a user who
/// removed Rift from their OS's startup UI isn't silently re-added — the
