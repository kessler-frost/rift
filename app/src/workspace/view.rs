...
use rift_util::path::{LineAndColumnArg, user_friendly_path};
...
use self::vertical_tabs::{
    ...
    render_settings_popup, vtab_group_position_id,
};
use super::action::{
...
use crate::tab::{
    MOVE_TO_GROUP_LABEL, NewSessionMenuItem, PaneNameMenuTarget, SelectedTabColor,
    TAB_BAR_BORDER_HEIGHT, TabBarState, TabComponent, TabData, tab_position_id, uses_vertical_tabs,
};
...
use crate::{GlobalResourceHandles, report_if_error, send_telemetry_from_ctx};
...
impl Workspace {
        ...
        ctx.spawn(async { cli_install::install_cli() }, |view, result, ctx| {
            match result {
                Ok(_) => {
                    ...
                    let message = format!("Successfully installed the Rift CLI! You can now run '{command_name}' from the command line.");
                ...
                Err(error) => {
                    let error_message = format!("Failed to install Rift command: {error}");
                    log::error!("{error_message}");
                    view.toast_stack.update(ctx, |toast_stack, ctx| {
                        let toast = DismissibleToast::error(error_message);
                        toast_stack.add_persistent_toast(toast, ctx);
    ...
    fn uninstall_cli(&mut self, ctx: &mut ViewContext<Self>) {
        ctx.spawn(
            ...
            |view, result, ctx| match result {
                Ok(_) => {
                    let message = "Successfully uninstalled the Rift command.";
                    view.toast_stack.update(ctx, |toast_stack, ctx| {
                        let toast = DismissibleToast::success(message.to_string());
                        toast_stack.add_ephemeral_toast(toast, ctx);
                    });
                ...
                Err(error) => {
                    let error_message = format!("Failed to uninstall Rift command: {error}");
                    log::error!("{error_message}");
                    view.toast_stack.update(ctx, |toast_stack, ctx| {
                        let toast = DismissibleToast::error(error_message);
                        toast_stack.add_persistent_toast(toast, ctx);
    ...
    fn refresh_working_directories_for_pane_group(
        ...
        let terminal_cwds: Vec<(EntityId, LocalOrRemotePath)> = pane_group
            ...
            .collect();
        // Code/diff/notebook panes were AI features and have been removed.
        let local_paths: Vec<(EntityId, LocalOrRemotePath)> = Vec::new();
    ...
    fn render_settings_error_banner(&self, app: &AppContext) -> Option<WorkspaceBannerFields> {
        ...
        // The "fix settings with AI" action was removed.
