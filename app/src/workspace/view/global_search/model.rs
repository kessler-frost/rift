...
use regex::escape;
use rift_ripgrep::search::{Match as RipgrepMatch, Submatch};
...
impl GlobalSearch {
    ...
    pub fn run_search(
        ...
        let handle = ctx.spawn(
            ...
            move |_, result, ctx| match result {
                ...
                Err(err) => {
                    log::error!("GlobalSearch: rift_ripgrep CLI search failed or aborted: {err}");
    ...
    async fn run_rift_ripgrep_cli(
        ...
        log::info!(
            "GlobalSearch: starting rift_ripgrep CLI search with pattern={pattern}, roots={:?}",
            roots_display
        );

        let stream =
            rift_ripgrep::search::search_streaming(&[pattern], &roots, ignore_case, multiline)?;
