...
use riftui::elements::{ChildView, Element};
...
use crate::workspace::view::{NotebookSource, OpenLeftPanelObjectSettings, Workspace};
...
impl Workspace {
    ...
    pub(super) fn build_open_in_rift_button(
        ctx: &mut ViewContext<Self>,
    ) -> ViewHandle<ActionButton> {
        ctx.add_typed_action_view(|_ctx| {
            ActionButton::new("Open in Rift", PrimaryTheme).on_click(move |ctx| {
                ...
                if let Some(url) = parse_current_url() {
                    ...
                    log::warn!("Could not get URL for Open in Rift button");
                }
            })
        })
    }
        ...
        ctx.subscribe_to_view(&panel, |me, _, event, ctx| match event {
            ...
            ConversationDetailsPanelEvent::OpenPlanNotebook { notebook_uid } => {
                me.open_notebook(
                    ...
                    &OpenLeftPanelObjectSettings::default(),
                    ctx,
                    true,
                );
            }
