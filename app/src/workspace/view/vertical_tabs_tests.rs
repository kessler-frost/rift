...
// user-set rename — leaving multi-pane tabs looking like they had no tab
// label at all. The new gate keeps the existing triggers and adds "any
// multi-pane tab", so every multi-pane group has a stable tab-level
// identifier in `Panes` granularity.
#[test]
...
fn tab_group_header_shows_for_multi_pane_tabs_without_custom_title() {
    // The #9098 case: a split tab that was never renamed. Each row only shows
    // the per-pane title, so without a group header there is no way to tell
    // two such tabs apart in the sidebar.
...
fn tab_group_header_distinguishes_two_auto_named_multi_pane_tabs() {
    // Models the screenshot in #9098: tab 1 has a custom title, tabs 2 and 3
    // are un-renamed multi-pane groups. Before the fix only tab 1 showed a
    // header; after the fix every multi-pane tab gets one so the user can
