...
/// only shown when a custom title existed, so a split tab that had never been
/// renamed rendered without any tab-level identifier — only its first row's
/// title was visible, which looked identical for every tab and made the bar
/// appear "nameless". Single-pane groups in `Panes` mode still omit the header
/// because the single row already shows the pane title (avoids duplicating the
/// same text immediately above itself).
