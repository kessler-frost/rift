...
cfg_if::cfg_if! {
    if #[cfg(feature = "completions_v2")] {
        ...
        use rift_js::FromRiftJs;
    }
}

/// Returns a JS object representing the Rift Plugin API exposed to external JavaScript plugins.
///
...
/// Returns a JS object representing the Completions namespace for the Rift Plugin API.
///
/// API methods:
///
        ...
        Function::new(
            ...
            MutFn::from(move |val: Value<'js>| {
                if val.is_array() {
                    ...
                    match Vec::<CommandSignature>::from_rift_js(
                        ctx,
                        val,
                    ...
                    let mut plugin = plugin.get_mut();
                    match CommandSignature::from_rift_js(
                        ctx,
                        val,
                        plugin.js_function_registry_mut(),
                    ) {
