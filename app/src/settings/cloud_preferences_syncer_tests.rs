...
async fn wait_for_num_spawned_futures(app: &mut App, expected_num: usize, message: &str) {
    for _ in 0..50 {
        ...
        riftui::r#async::Timer::after(Duration::from_millis(100)).await;
    }
...
async fn drain_sync_queue(app: &mut App) {
    ...
    // to a cloud change).
    for _ in 0..5 {
        riftui::r#async::Timer::after(Duration::from_millis(200)).await;
...
fn write_settings_file_with_content(path: &std::path::Path, content: &str) -> String {
    ...
    riftui_extras::user_preferences::toml_backed::TomlBackedUserPreferences::file_content_hash(path)
