...
use crate::{report_if_error, send_telemetry_from_ctx, GlobalResourceHandlesProvider};
...
impl SettingsImportView {
    ...
    fn render_reset_button(&self, appearance: &Appearance) -> Box<dyn riftui::Element> {
        appearance
            ...
            .with_centered_text_label("Reset to Rift defaults".to_owned())
            .build()
            .on_click(move |ctx, _, _| {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            SettingsImportAction::ImportButtonClicked => {
                ...
                // set_preferences should not fail because it is writing directly to Rift's preferences.
