...
mod appimage {
    ...
    pub(super) fn relaunch(appimage_path: &Path) -> Result<()> {
        ...
        // autoupdate process.
        command.arg(rift_cli::finish_update_flag());
        ...
        if let Ok(path) = std::env::var("RIFT_CHANNEL_VERSIONS_PATH") {
            command.env("RIFT_CHANNEL_VERSIONS_PATH", path);
        }
...
mod package_manager {
    ...
    use riftui::elements::{Container, FormattedTextElement, HighlightedHyperlink};
    use riftui::{Element, SingletonEntity as _};
    ...
    impl riftui::Entity for AutoupdateContextBlock {
        type Event = ();
    }

    impl riftui::View for AutoupdateContextBlock {
    ...
    pub(super) fn relaunch() -> Result<()> {
        ...
        // finishing an update.
        let finish_update_flag = rift_cli::finish_update_flag();
        command.args(
            std::env::args()
                .skip(1)
        ...
        let base_command = match self {
            ...
            PackageManager::Pacman {
                ...
                let repo_prefix = if !is_repo_configured {
                    let cache_dir = rift_core::paths::cache_dir();
