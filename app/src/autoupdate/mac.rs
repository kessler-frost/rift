...
use rift_core::macos::get_bundle_path;
...
fn old_executable_file_path() -> PathBuf {
    rift_core::paths::state_dir().join(OLD_EXECUTABLE_FILE_NAME)
}
...
pub(super) fn relaunch() -> Result<()> {
    ...
    // autoupdate process.
    launch_command.push(format!(" --args {}", rift_cli::finish_update_flag()));
    ...
    if let Ok(path) = env::var("RIFT_CHANNEL_VERSIONS_PATH") {
        launch_command.push(format!(" --env RIFT_CHANNEL_VERSIONS_PATH={path}"));
    }
...
pub async fn cleanup_all_except(preserve_update_id: Option<&str>) {
    let mut autoupdate_dir = rift_core::paths::cache_dir();
...
fn get_download_dir(update_id: &str) -> PathBuf {
    let mut dir = rift_core::paths::cache_dir();
    dir.push("autoupdate");
    dir.push(update_id);
    dir
}
