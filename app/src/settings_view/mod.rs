...
pub mod flags {
    ...
    pub const SSH_RIFTIFICATION_CONTEXT_FLAG: &str = "SSH_Riftification";
...
pub struct SettingsView {
    ...
    /// per `SettingsView` per `RIFT.md`'s guidance that inline
...
impl SettingsView {
        ...
        match event {
            EditorEvent::Edited(_) => {
                ...
                if is_search_active {
                    ...
                    // No multi-subpage backing pages remain, so there is no
                    // per-subpage filtering to run.
    ...
    pub fn set_and_refresh_current_page_internal(
        ...
        // Check the backing page for this section exists.
        ...
        // When navigating to a subpage, auto-expand the umbrella containing it.
        ...
        match action {
            ...
            SettingsAction::RiftifyPageToggle(riftify_action) => {
                if let Some(riftify_page) = self.settings_page(SettingsSection::Riftify)
                    && let SettingsPageViewHandle::Riftify(view) = &riftify_page.view_handle
                {
                    view.update(ctx, |view, ctx| {
