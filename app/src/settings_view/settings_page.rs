...
use rift_core::settings::SyncToCloud;
...
impl LocalOnlyIconState {
    ...
    pub fn for_setting(
        ...
        // Cloud settings sync has been removed; all settings are local-only,
        // so the "not cloud-synced" indicator is never shown.
        Self::Hidden
    }
}
...
/// [`SettingsWidget`]s.
pub(super) enum PageType<V: riftui::View> {
...
pub(super) enum FilteredPageType<'a, V: riftui::View> {
    Monolith {
...
pub(super) struct Category<V: riftui::View> {
    title: &'static str,
    subtitle: Option<&'static str>,
    widgets: Vec<Box<dyn SettingsWidget<View = V>>>,
}

impl<V: riftui::View> Category<V> {
    pub(super) fn new(
        title: &'static str,
        widgets: Vec<Box<dyn SettingsWidget<View = V>>>,
    ) -> Self {
...
pub(super) struct FilteredCategory<'a, V: riftui::View> {
