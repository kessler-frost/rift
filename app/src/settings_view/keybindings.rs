...
use riftui::keymap::{DescriptionContext, Keystroke, Trigger};
...
impl KeybindingsView {
    ...
    fn remove_keystroke(&mut self, index: usize, ctx: &mut ViewContext<Self>) {
        if let Some(row) = self.rows.as_mut().and_then(|rows| rows.get_mut(index)) {
            ...
            // Persist the keybinding into the `.rift` directory so that it will last beyond
            // this session
...
impl SettingsWidget for KeybindingsWidget {
    ...
    fn render(
        ...
        // Cloud settings sync has been removed; keyboard shortcuts are always
        // local-only, so the "not synced to cloud" indicator is never shown.
