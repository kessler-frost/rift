...
/// across renders per `RIFT.md`; the scroll state handle serves the same
...
pub fn render_open_settings_file_button(
    ...
    Hoverable::new(mouse_state, move |state| {
        let icon = ConstrainedBox::new(Icon::Code2.to_riftui_icon(text_fill).finish())
...
pub fn render_settings_error_alert(
    ...
    let alert_icon = ConstrainedBox::new(
        Icon::AlertCircle
            .to_riftui_icon(Fill::Solid(text_color))
            .finish(),
    )
    .with_width(FOOTER_ICON_SIZE)
    .with_height(FOOTER_ICON_SIZE)
    ...
    // `ClippedScrollable` wants `riftui::elements::Fill` (not the theme
    ...
    let scrollable_text = ConstrainedBox::new(
        ClippedScrollable::vertical(
            ...
            text_row,
            ScrollbarWidth::Auto,
            riftui::elements::Fill::Solid(coloru_with_opacity(text_color, 30)),
            riftui::elements::Fill::Solid(coloru_with_opacity(text_color, 60)),
