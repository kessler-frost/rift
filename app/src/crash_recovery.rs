...
use rift_cli::RecoveryMechanism;
...
/// Keep in sync with [`rift_cli::AppArgs`].
...
pub fn is_crash_recovery_process(args: &rift_cli::AppArgs) -> bool {
...
impl CrashRecoveryProcess {
    ...
    fn kill(&mut self) {
        ...
        *IS_CRASH_RECOVERY_PROCESS_RUNNING.write() = false;
        self.is_alive = false;
        rift_logging::on_crash_recovery_process_killed();
    }
...
impl CrashRecovery {
    ...
    #[cfg(test)]
    pub fn register_for_test(app: &mut riftui::App) {
...
fn handle_parent_crash(
    ...
    rift_logging::on_parent_process_crash();
