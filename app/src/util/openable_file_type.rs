//! File type detection utilities for determining if files can be opened in Rift.
...
/// The type of file that can be opened in Rift. The in-product treatment for "opening" a file
/// depends on its type.
...
pub enum FileTarget {
    /// Open in Rift's Markdown viewer.
    MarkdownViewer(EditorLayout),
    /// Open in Rift's Code Editor.
...
/// "Open with Rift" is invoked from Finder/another app via a `file://` URL.
///
...
pub fn is_runnable_shell_script(path: &Path) -> bool {
    ...
    // A file `chmod 070` belongs to a group, not to the user invoking Rift.
