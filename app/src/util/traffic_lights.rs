...
//! depends on the platform. The Rift app must use this information to avoid rendering UI elements
//! underneath them.
...
mod windows_only {
    pub(super) use pathfinder_color::ColorU;
    pub(super) use pathfinder_geometry::vector::vec2f;
    pub(super) use rift_core::ui::theme;
    pub(super) use riftui::elements::{
...
use riftui::{AppContext, Element, WindowId};
#[cfg(target_os = "windows")]
use windows_only::*;
...
/// Data the Rift app needs to avoid rendering anything below the traffic lights.
#[derive(Clone, Debug)]
...
impl TrafficLightData {
    ...
    /// by riftui. However, native traffic light buttons (e.g. on macOS) don't scale with zoom, so
