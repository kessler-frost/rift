...
pub async fn execute_command(
    ...
    // when Rift is launched via launchd (Finder, Dock) with a minimal environment.
...
// (fastfetch, MOTD banners, a stray echo, etc.) can't be mistaken for PATH content.
#[cfg(feature = "local_tty")]
const PATH_CAPTURE_START: &str = "__RIFT_PATH_CAPTURE_START__";
#[cfg(feature = "local_tty")]
const PATH_CAPTURE_END: &str = "__RIFT_PATH_CAPTURE_END__";
...
async fn capture_interactive_shell_env(
    ...
    // controlling terminal with `tcsetpgrp` [1]. If the Rift process itself tries to read from
