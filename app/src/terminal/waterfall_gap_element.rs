...
use riftui::elements::{ScrollData, ScrollableElement, ZIndex};
use riftui::event::ModifiersState;
use riftui::units::{IntoLines, IntoPixels, Pixels};
use riftui::{AppContext, Element, EventContext, ModelHandle, SizeConstraint};
...
pub struct WaterfallGapElement {
    ...
    origin: Option<riftui::elements::Point>,
...
impl Element for WaterfallGapElement {
    fn layout(
        &mut self,
        constraint: riftui::SizeConstraint,
        ctx: &mut riftui::LayoutContext,
        app: &AppContext,
    ) -> Vector2F {
    ...
    fn after_layout(&mut self, ctx: &mut riftui::AfterLayoutContext, app: &AppContext) {
        self.input_element.after_layout(ctx, app);
        self.block_list_element.after_layout(ctx, app);
    }

    fn paint(&mut self, origin: Vector2F, ctx: &mut riftui::PaintContext, app: &AppContext) {
        self.origin = Some(riftui::elements::Point::from_vec2f(
            origin,
            ctx.scene.z_index(),
        ));
    ...
    fn origin(&self) -> Option<riftui::elements::Point> {
        self.origin
    }
    ...
    fn dispatch_event(
        ...
        if !handled {
            ...
            handled |= match event_at_z_index {
                Some(riftui::Event::LeftMouseDown { position, .. }) => {
                    self.mouse_down(*position, ctx)
                }
                Some(riftui::Event::RightMouseDown { position, .. }) => {
...
impl ScrollableElement for WaterfallGapElement {
    fn scroll_data(&self, app: &AppContext) -> Option<riftui::elements::ScrollData> {
