...
/// restore the PS1 from the saved PS1 value (we had unset the PS1 for Rift prompt).
...
impl<T: EventLoopSender> PtyController<T> {
    pub fn new(
        ...
        ctx.subscribe_to_model(&model_event_dispatcher, |me, event, ctx| match event {
            ...
            ModelEvent::HonorPS1OutOfSync => {
                // We force re-sync the PS1 state of Rift settings with the shell's environment variable, $RIFT_HONOR_PS1, via
                ...
                if honor_ps1 {
                    ...
                    me.send_switch_to_rift_prompt_bindkey(ctx);
                }
            }
    ...
    /// Sends bindkey to notify shell process to switch to Rift prompt logic for prompt
...
fn bytes_to_execute_command(
    ...
    // Normalize line endings up-front based on the shell family. POSIX shells
    // expect `\n`, so on Windows we collapse CRLF/CR to `\n`; PowerShell ignores
    // `\n` and needs `\r` to execute multi-line commands (PLAT-770). POSIX
    // shells on non-Windows platforms keep their bytes untouched.
