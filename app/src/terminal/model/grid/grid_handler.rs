...
use rift_core::semantic_selection::{SMART_SELECT_MATCH_WINDOW_LIMIT, SemanticSelection};
use rift_core::{safe_assert, safe_assert_eq};
pub use rift_terminal::model::TermMode;
use rift_terminal::model::grid::{CellType, FlatStorage};
...
fn is_url_link_separator(c: char) -> bool {
...
/// grid that needs to be kept in sync with Rift's grids. We do this via clearing the ConPTY
/// grid before Rift starts populating a new grid.
///
...
impl GridHandler {
    ...
    pub(super) fn input_at_cursor(&mut self, text: &str) {
        use rift_terminal::model::VisiblePoint;
