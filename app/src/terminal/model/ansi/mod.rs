...
use log::debug;
pub use rift_terminal::model::ansi::control_sequence_parameters::*;
...
const RIFT_COMPLETIONS_OSC_MARKER: &[u8] = b"9280";
const RIFT_COMPLETIONS_START_BYTE: &[u8] = b"A";
const RIFT_COMPLETIONS_END_BYTE: &[u8] = b"B";
const RIFT_COMPLETIONS_MATCH_RESULT_BYTE: &[u8] = b"C";
...
const RIFT_COMPLETIONS_MATCH_UPDATE_METADATA: &[u8] = b"D?";

/// Marks an OSC that tells the terminal that the shell is ready to receive
/// the the string to run completions for.
const RIFT_COMPLETIONS_PROMPT_BYTE: &[u8] = b"P";

const RIFT_KV_START_BYTE: &[u8] = b"A";
const RIFT_KV_ENTRY_BYTE: &[u8] = b"B";
const RIFT_KV_END_BYTE: &[u8] = b"C";
        ...
        while !remaining.is_empty() {
            ...
            for (idx, byte) in remaining.iter().enumerate() {
                ...
                if was_sync_output != is_sync_output {
                    // Synchronized output was toggled: the pre- and post-toggle
                    // bytes should be handled in separate
                    // `on_finish_byte_processing` calls.
                    split_index = idx + 1;
                    break;
                }
            }
            handler.on_finish_byte_processing(&ProcessorInput {
                bytes: &remaining[..split_index],
                ..input
            });
...
impl<'a, H: Handler + 'a, W: io::Write> Performer<'a, H, W> {
    ...
    fn handle_decoded_hook(&mut self, hook: Result<DProtoHook, serde_json::Error>) {
        ...
        match hook {
            ...
            Ok(DProtoHook::SourcedRcFileForRift { .. }) => {
                ...
                // debugability.
                log::error!("Received hex-encoded SourcedRcFileForRift escape sequence.");
            }
            Ok(DProtoHook::FinishUpdate { value }) => self.handler.finish_update(value),
            Ok(DProtoHook::RemoteRiftificationIsUnavailable { value }) => {
                self.handler.remote_riftification_is_unavailable(value)
            }
    ...
    fn handle_unencoded_hook(&mut self, hook: Result<DProtoHook, serde_json::Error>) {
        // Currently, only the `SourcedRcFileForRift`, `InitShell`, `InitSubshell`, and `InitSsh`
        ...
        match hook {
            ...
            Ok(DProtoHook::SourcedRcFileForRift { value }) => {
            ...
            Ok(_) => {
                log::error!("Received non hex-encoded hook that is not SourcedRcFileForRift");
            }
            Err(err) => {
                log::warn!("Received malformed SourcedRcFileForRift hook {err:#}");
            }
        }
    }
    ...
    fn osc_dispatch(&mut self, params: &[&[u8]], bell_terminated: bool) {
        ...
        match params[0] {
            ...
            RIFT_COMPLETIONS_OSC_MARKER => match params.get(1) {
                ...
                Some(&RIFT_COMPLETIONS_END_BYTE) => {
                    self.handler.end_completions_output();
                }
                Some(&RIFT_COMPLETIONS_MATCH_RESULT_BYTE) => {
                    // The payload for the OSC is contained in the third parameter.
                    let Some(data_str) = params
                        .get(2)
                        .map(|osc_data| String::from_utf8_lossy(osc_data))
                ...
                Some(bytes) if bytes.starts_with(RIFT_COMPLETIONS_MATCH_UPDATE_METADATA) => {
                    ...
                    match &parameter[2..] {
                        ...
                        _ => {
                            log::warn!(
                                "Invalid Rift OSC marker parameter for completions match metadata: {parameter}"
                            );
                        }
                    }
                }
                Some(&RIFT_COMPLETIONS_PROMPT_BYTE) => {
