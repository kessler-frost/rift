...
/// Indicates that the following JSON-encoded message is hex-encoded for Rift's lifecycle hooks.
...
/// Indicates that the following JSON-encoded message is unencoded for Rift's lifecycle hooks.
/// In DCS, it is used as the final char in the DCS start sequence.
/// In OSC, it is used as the first parameter.
pub(super) const UNENCODED_JSON_MARKER: char = 'f';

/// Indicates that the following message is a ANSI-C quoted message for receiving Rift's lifecycle
...
/// Enum representing all possible JSON payloads for Rift's DCS's.
...
impl DProtoHook {
    ...
    pub fn session_id(&self) -> Option<SessionId> {
        match self {
            ...
            DProtoHook::SourcedRcFileForRift { .. }
            | DProtoHook::RemoteRiftificationIsUnavailable { .. }
...
/// Received from the pty after the shell has finished executing Rift's
/// bootstrap script.
...
/// Received from the pty when Rift's SSH wrapper is executed, prior to
...
/// The binding is triggered when Rift writes the `ESC-i` escape sequence to the pty.
/// Rift usually does this after a block completes, to collect any typeahead
...
/// Received from the pty when rift_finish_update is called at the end of an
...
/// `logout`, Ctrl-D on an empty prompt, etc.). Lets the Rift client drop
