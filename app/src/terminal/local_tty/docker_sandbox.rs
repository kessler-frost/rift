...
//!
//! This module owns everything specific to running a Rift shell inside a
...
///
/// Lives under the Rift per-user cache directory rather than `/tmp` so:
...
fn docker_sandbox_host_root() -> PathBuf {
    rift_core::paths::cache_dir().join("docker-sandbox")
}

/// Resolves the absolute path to the `sbx` CLI binary using the Rift
/// process's `PATH`.
///
/// Rift's process `PATH` is minimal and often misses user-shell-installed
/// tools (e.g. homebrew on Apple Silicon when Rift is launched from Finder,
...
///
/// Each instance carries a unique `sandbox_id` so multiple Rift panes can run
...
impl DockerSandboxShellStarter {
    ...
    /// Host directory where Rift writes this sandbox's bash init script.
