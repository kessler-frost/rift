...
use rift_core::ui::color::{coloru_with_opacity, Opacity};
...
pub fn calculate_scaled_font_size(appearance: &rift_core::ui::appearance::Appearance) -> f32 {
...
pub fn calculate_max_profile_name_width(appearance: &rift_core::ui::appearance::Appearance) -> f32 {
        ...
        ctx.subscribe_to_model(&input_model, move |_me, _, event, ctx| match event {
            ...
            | BlocklistAIInputEvent::LockChanged { config } => {
                if config.is_locked && !config.input_type.is_ai() {
                    ...
                    ctx.spawn(
                        riftui::r#async::Timer::after(NEW_MODEL_CHOICES_POPUP_DELAY),
                        |_, _, ctx| {
                            ctx.notify();
                        },
                    );
...
impl View for ProfileModelSelector {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let is_wasm_mobile = riftui::platform::is_mobile_device();
