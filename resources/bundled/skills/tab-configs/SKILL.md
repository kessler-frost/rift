---
name: tab-configs
description: Reference the Rift tab config schema, validation rules, and examples. Use when creating or updating Rift tab config TOML files or when another tab-config skill needs the canonical schema details.
---
...
- When creating or locating tab configs, use the correct tab config directory for the user's current Rift build/channel rather than hardcoding a single base directory.
...
`{{autogenerated_branch_name}}` is a special-cased reserved template variable: if a tab config references it, Rift generates a unique worktree branch name on each open instead of prompting the user for that value.
...
directory = "{{repo}}"
commands = [
  "git worktree add -b {{branch_name}} $HOME/.rift/worktrees/$(basename {{repo}})/{{branch_name}} {{base_branch}}",
  "cd $HOME/.rift/worktrees/$(basename {{repo}})/{{branch_name}}",
]
