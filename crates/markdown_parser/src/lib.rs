...
/// This purposefully shadows the `Action` trait from `riftui`.
///
/// Since `riftui` depends on this crate, we can't depend on the `riftui_core::Action` trait directly.
/// Instead, we create a new trait with a blanket implementation that implicitly results
/// in any `riftui_core::Action` implementing this `Action`.
...
impl FormattedTable {
    /// Parse from the internal tab-separated format used in `rift-markdown-table` code blocks.
    ...
    /// Serialize to the internal tab-separated format used in `rift-markdown-table` code blocks.
