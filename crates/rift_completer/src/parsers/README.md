...
Let's say we are interested in parsing the input `rift --disable-telemetry`. Command calls generally have the form `cmd <arg1> <arg2> <argN>` where `<arg>`s are positional parameters. Let's use the primary functions of the parser and inspect. The first step we do is calling the tokenizer (the function `lex` here):

```rust
let input = "rift --disable_telemetry";
let start_offset = 0;
...
let example = Spanned { item: String::from("rift"), span: Span::new(0,4) };
assert_eq!(example.item, "rift".to_string());
assert_eq!(example.span, Span::new(0,4));

let example = String::from("rift").spanned(Span::new(0,4));
assert_eq!(example.item, "rift".to_string());
assert_eq!(example.span, Span::new(0,4));

let example = "rift -p --disable-telemetry";
...
let expected_word1 = String::from("rift").spanned(Span::new(0,4));
...
For more involved inputs (say commands separated by `|` and/or having `;`) the lite parser will effectively create the necessary `LitePipeline`s to express it, let's explore what happens if we lite parse the input `rift config-set --extension-path="/path/to/dir" ; echo $RIFT_VAR"` (*two pipelines here due to the ; character*), like so:

```rust
let input = "rift config-set --extension-path=\"/path/to/dir\" ; echo $RIFT_VAR";
let start_offset = 0;
