...
impl Element for ScrollableElement {
    ...
    fn after_layout(&mut self, _: &mut riftui::AfterLayoutContext, _: &AppContext) {}

    fn paint(&mut self, origin: Vector2F, ctx: &mut riftui::PaintContext, _app: &AppContext) {
    ...
    fn dispatch_event(
        &mut self,
        _: &riftui::event::DispatchedEvent,
        _: &mut riftui::EventContext,
        _: &AppContext,
    ) -> bool {
        false
    }
...
impl NewScrollableElement for ScrollableElement {
    ...
    fn scroll(&mut self, delta: riftui::units::Pixels, axis: Axis, ctx: &mut riftui::EventContext) {
        match axis {
            Axis::Horizontal => (),
            Axis::Vertical => {
