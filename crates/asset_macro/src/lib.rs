//! This module defines a set of macros used to reference assets in Rift.
//!
...
use rift_util::assets::{ASSETS_DIR, ASYNC_ASSETS_DIR, BUNDLED_ASSETS_DIR, REMOTE_ASSETS_DIR};
use sha2::Digest;
use syn::parse::Parse;
...
fn construct_bundled_asset(asset_name: &str, asset_dir: &str) -> Result<TokenStream2, String> {
    if full_asset_path(asset_name, asset_dir).exists() {
        ...
        Ok(quote! {
            ::riftui_core::assets::asset_cache::AssetSource::Bundled {
...
fn construct_remote_asset(asset_name: &str, asset_dir: &str) -> Result<TokenStream2, String> {
    ...
    let url = rift_util::assets::hashed_asset_url(&rift_util::assets::hashed_asset_path(
        Path::new(asset_name),
        &hash,
    ));

    Ok(quote! {
        ::asset_cache::url_source(::rift_util::assets::make_absolute_url( #url ))
    })
}
