...
use riftui_core::{async_assert, async_assert_eq, App};
...
/// Test that clicking a file in the file tree opens it in Rift's editor.
/// This is a regression test for the bug where files were being opened in
/// external editors instead of Rift's built-in editor.
pub fn test_file_tree_opens_files_in_rift() -> Builder {
    new_builder()
        .with_setup(|utils| {
...
pub fn test_file_tree_non_openable_files() -> Builder {
    new_builder()
        .with_setup(|utils| {
            ...
            // Create a binary file that shouldn't be opened in Rift
        ...
        .with_step(
            new_step_with_default_assertions("Click on binary file")
                ...
                .add_assertion(|app, window_id| {
                    // The binary file should NOT open in a new pane in Rift
