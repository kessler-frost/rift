...
macro_rules! generate_can_bootstrap_tmux_ssh_test_for_shell {
    ($fn_name:ident, $shell:literal, $install_tmux:literal) => {
        ...
        /// successfully.
        pub fn $fn_name() -> Builder {
            fn riftify(builder: Builder) -> Builder {
                builder
                    .with_step(enter_ssh_command($shell))
                    .with_step(wait_for_password_prompt(0 /*tab_idx*/, $shell))
                    .with_step(
            ...
            fn assert_riftification(builder: Builder) -> Builder {
                builder
            ...
            let builder = new_builder()
                ...
                .with_step(setup_gcloud_sdk());
            // Install Tmux
            let builder = riftify(builder).with_step(
                accept_tmux_install().set_post_step_pause(std::time::Duration::from_secs(3)),
            );
            // Quit SSH Session once we validate riftification works with Tmux Install
            let builder = assert_riftification(builder).with_step(run_exit_command());

            // Validate we can Riftify when Tmux is already installed
            assert_riftification(riftify(builder))
        }
    };
}
