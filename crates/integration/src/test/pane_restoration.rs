    ...
    new_builder()
        ...
        .with_step(
            new_step_with_default_assertions("Check pane count before undo close")
                .add_assertion(move |app, window_id| {
                    let workspace_view = rift::integration_testing::view_getters::workspace_view(app, window_id);
                    ...
                    riftui_core::async_assert_eq!(initial_pane_count, 1, "Should have exactly one pane after grace period expires - closed pane should be cleaned up")
                }),
        )
        .with_step(trigger_undo_close()
            ...
            .add_assertion(move |app, window_id| {
                ...
                workspace_view.read(app, |workspace, ctx| {
                    ...
                    riftui_core::async_assert_eq!(pane_count, 1, "Should still have only one pane after attempted restore - no pane should be restored")
                })
            }),
        )
        .with_step(
...
pub fn test_closed_panes_cleared_on_rearrangement() -> Builder {
    ...
    new_builder()
        ...
        .with_step(
            ...
            rift::integration_testing::pane_group::close_pane_by_index(
                0, // tab index
                ...
                .add_assertion(move |app, window_id| {
                    ...
                    workspace_view.read(app, |workspace, ctx| {
                        ...
                        if visible_pane_count != 2 {
                            riftui_core::integration::AssertionOutcome::failure(format!("Should have 2 visible panes after closing one (got {visible_pane_count} visible, {total_pane_count} total)"))
                        } else {
                            riftui_core::integration::AssertionOutcome::Success
                        }
                    })
                }),
        )
        .with_step(
            ...
            rift::integration_testing::pane_group::move_pane_by_indices(
                0,
                0,
                1,
                rift::pane_group::tree::Direction::Right,
            ),
        )
        .with_step(
                ...
                .add_assertion(move |app, window_id| {
                    ...
                    workspace_view.read(app, |workspace, ctx| {
                        ...
                        if visible_pane_count < 1 {
                            riftui_core::integration::AssertionOutcome::failure(format!("Should have at least 1 visible pane after undo close attempt, got {visible_pane_count}"))
                        } else {
                            riftui_core::integration::AssertionOutcome::Success
                        }
                    })
        ...
        .with_step(
            trigger_undo_close().add_assertion(move |app, window_id| {
                ...
                workspace_view.read(app, |workspace, ctx| {
                    ...
                    if visible_pane_count < 1 {
                        riftui_core::integration::AssertionOutcome::failure(format!("Should have at least 1 visible pane after second undo close attempt, got {visible_pane_count}"))
                    } else {
                        riftui_core::integration::AssertionOutcome::Success
                    }
                })
                ...
                .add_assertion(move |app, window_id| {
                    ...
                    if visible_pane_count == 1 {
                        ...
                        match (original_result, third_result) {
                            (riftui_core::integration::AssertionOutcome::Success, riftui_core::integration::AssertionOutcome::Success) => {
                                riftui_core::integration::AssertionOutcome::Success
                            }
                            _ => riftui_core::integration::AssertionOutcome::failure("Expected to find both original_pane and third_pane content".to_string())
                        }
                    } else {
                        riftui_core::integration::AssertionOutcome::failure(format!("Unexpected pane count: {visible_pane_count}"))
                    }
                }),
        )
}
    ...
    new_builder()
        ...
        .with_step(
            new_step_with_default_assertions("Verify restored pane has correct content in new tab")
                ...
                .add_assertion(move |app, window_id| {
                    let workspace_view =
                        rift::integration_testing::view_getters::workspace_view(app, window_id);
