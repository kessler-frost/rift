...
use riftui_core::integration::{TestStep, ARTIFACTS_DIR_ENV_VAR};
...
///
/// ```sh
/// RIFTUI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS=1 \
            ...
            move |_app, _window_id, _persisted_data| -> Pin<Box<dyn Future<Output = ()> + Send>> {
                Box::pin(async move {
                    let artifacts_root = std::env::var(ARTIFACTS_DIR_ENV_VAR)
                        ...
                        .unwrap_or_else(|_| {
                            std::env::temp_dir().join("rift_integration_test_artifacts")
                        });
