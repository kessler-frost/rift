...
const EXTRA_HTTP_HEADERS_ENV_VAR: &str = "RIFT_EXTRA_HTTP_HEADERS";
    ...
    fn add_rift_http_headers(mut builder: RequestBuilder) -> RequestBuilder {
        ...
        if let Ok(os_system_info) = OperatingSystemInfo::get() {
            ...
            if let Ok(category) = HeaderValue::from_str(&category) {
                builder = builder.header(headers::RIFT_OS_CATEGORY, category);
            }

            // Operating system name.
            builder = builder.header(
                headers::RIFT_OS_NAME,
            ...
            {
                builder = builder.header(headers::RIFT_OS_VERSION, version);
            }
            ...
            {
                builder =
                    builder.header(headers::RIFT_OS_LINUX_KERNEL_VERSION, linux_kernel_version);
            }
        }
...
// NOTE: the `origin_tests` module was removed during the AI/cloud strip. It
// exercised `is_rift_server_origin` + `ChannelState::server_root_url`/`rtc_http_url`,
// all of which were the cloud-server origin-detection helpers that no longer
// exist now that the server/cloud layer is gone.
