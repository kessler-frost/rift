...
fn test_parse_user_default() {
    let result = parse_share_request("ben@rift.dev").unwrap();
    match result.subject {
        ShareSubject::User { email } => assert_eq!(email, "ben@rift.dev"),
...
fn test_parse_user_view() {
    let result = parse_share_request("ben@rift.dev:view").unwrap();
    match result.subject {
        ShareSubject::User { email } => assert_eq!(email, "ben@rift.dev"),
        _ => panic!("Expected User subject"),
    }
...
fn test_parse_user_edit() {
    let result = parse_share_request("ben@rift.dev:edit").unwrap();
    match result.subject {
        ShareSubject::User { email } => assert_eq!(email, "ben@rift.dev"),
        _ => panic!("Expected User subject"),
    }
