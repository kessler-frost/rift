...
use rift_core::errors::{AnyhowErrorExt, ErrorExt, register_error};
...
use rift_graphql::mutations::expire_api_key::{
    ExpireApiKey, ExpireApiKeyResult, ExpireApiKeyVariables,
};
use rift_graphql::mutations::generate_api_key::{
    GenerateApiKey, GenerateApiKeyInput, GenerateApiKeyResult, GenerateApiKeyVariables,
};
use rift_graphql::mutations::mint_custom_token::{MintCustomTokenResult, MintCustomTokenVariables};
use rift_graphql::mutations::set_user_is_onboarded::{
    SetUserIsOnboarded, SetUserIsOnboardedResult, SetUserIsOnboardedVariables,
};
use rift_graphql::mutations::update_user_settings::{
    UpdateUserSettings, UpdateUserSettingsInput, UpdateUserSettingsResult,
    UpdateUserSettingsVariables,
};
use rift_graphql::queries::api_keys::{
    ApiKeyProperties, ApiKeyPropertiesResult, ApiKeys, ApiKeysVariables,
};
use rift_graphql::queries::get_user::{GetUser, GetUserVariables, UserOutput as GqlUserOutput};
use rift_graphql::queries::get_user_settings::{GetUserSettings, GetUserSettingsVariables};
use rift_server_auth::credentials::{AuthToken, Credentials, FirebaseToken, LoginToken};
pub use rift_server_auth::user_uid;
pub use session::*;
use thiserror::Error;
...
pub trait AuthClient: Send + Sync {
    ...
    async fn create_api_key(
        ...
        expires_at: Option<rift_graphql::scalars::Time>,
...
impl AuthClientImpl {
    ...
    async fn update_settings(
        ...
        let operation = UpdateUserSettings::build(UpdateUserSettingsVariables {
            input,
            request_context: rift_graphql::client::get_request_context(),
        });
    ...
    fn on_settings_updated(
        ...
        match result {
            ...
            UpdateUserSettingsResult::UserFacingError(error) => Err(anyhow!(
                rift_graphql::client::get_user_facing_error_message(error)
            )),
...
impl AuthClient for AuthClientImpl {
    async fn create_anonymous_user(
        ...
        let operation = CreateAnonymousUser::build(CreateAnonymousUserVariables {
            input: rift_graphql::mutations::create_anonymous_user::CreateAnonymousUserInput {
                anonymous_user_type,
                expiration_type: rift_graphql::mutations::create_anonymous_user::AnonymousUserExpirationType::NoExpiration,
                referral_code,
            },
            request_context: rift_graphql::client::get_request_context(),
        });
    ...
    async fn fetch_new_custom_token(&self) -> Result<MintCustomTokenResult> {
        let operation = rift_graphql::mutations::mint_custom_token::MintCustomToken::build(
    ...
    async fn fetch_user_properties<'a>(
        ...
        match response.user {
            rift_graphql::queries::get_user::UserResult::UserOutput(user_output) => Ok(user_output),
            rift_graphql::queries::get_user::UserResult::Unknown => {
    ...
    async fn get_user_settings(&self) -> Result<Option<SyncedUserSettings>> {
        ...
        let response = send_graphql_request(self.base_client.as_ref(), operation, None).await?;
        match response.user {
            rift_graphql::queries::get_user_settings::UserResult::UserOutput(user_output) => {
                Ok(user_output
                    .user
                    .settings
            ...
            rift_graphql::queries::get_user_settings::UserResult::Unknown => {
