//! Node.js and npm runtime management for Rift.
//!
...
pub fn node_installation_dir() -> Result<PathBuf> {
    ...
    Ok(rift_core::paths::data_dir().join("node").join(folder_name))
}
...
/// 3. Extract it to the Rift data directory
///
/// # Returns
...
pub async fn install_npm(client: &http_client::Client) -> Result<PathBuf> {
    ...
    let node_containing_dir = rift_core::paths::data_dir().join("node");
...
/// 1. First, our custom Node.js installation in the Rift data directory
...
pub async fn detect_system_node(path_env_var: impl AsRef<OsStr>) -> Result<()> {
    ...
    // via Rift's inherited env rather than the captured interactive PATH.
    #[cfg(windows)]
