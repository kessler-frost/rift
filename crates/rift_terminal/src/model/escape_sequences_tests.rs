...
fn test_keystroke_to_c0_control_code() {
    ...
    let test_cases: &[(Keystroke, Vec<u8>)] = &[
        ...
        // `ctrl-/` is an xterm-era addition to the VT220 table. Upstream warp GH#4620.
...
fn test_ctrl_slash_emits_us_control_code() {
    // Regression test for upstream warp GH#4620: `ctrl-/` must reach the pty as US (0x1f), which
    // is what Vim/Neovim's `<C-/>` mappings listen for. No platform keyboard layer folds `ctrl-/`
    // into a control byte, so without an explicit mapping a bare `/` was sent instead.
    let terminal_model_mock = TerminalModelMock::new();
...
/// Regression test for macOS editing keys under the Kitty keyboard protocol (upstream warp GH#9159).
///
...
fn test_kitty_protocol_mac_option_space_composition_is_not_disambiguated() {
    if !riftui_core::platform::OperatingSystem::get().is_mac() {
        return;
    }
