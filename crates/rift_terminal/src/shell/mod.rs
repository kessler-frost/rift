...
#[cfg(windows)]
use rift_core::paths::base_config_dir;
...
impl Shell {
    ...
    ///
    /// These sequences are bound to Rift shell functions during session bootstrap that print the
...
impl From<ShellType> for rift_util::path::ShellFamily {
...
impl ShellType {
    ...
    /// on the target (e.g. via SSH or Auto-Riftify). A plain `PathBuf` would pick the
    ...
    /// If true, Rift will bootstrap the shell if it's the login shell on the remote host.
                        ...
                        output_string
                            ...
                            .flat_map(|line| {
                                ...
                                for ext in PATHEXT {
                                    ...
                                    // Rift about this command as-is and also sans-suffix, e.g.
                                    // "git" and "git.exe".
...
impl ShellLaunchData {
    ...
    fn to_native_path_encoding(&self, path_str: &str) -> Option<PathBuf> {
        match self {
            ...
            // The container is Unix; Rift runs on the host, so paths are
