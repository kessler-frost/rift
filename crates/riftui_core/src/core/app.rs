...
use crate::{
    ...
    AnyView, ApplicationBundleInfo, Clipboard, CursorInfo, Effect, Element, Entity, EntityId,
    Event, GetSingletonModelHandle, ModelAsRef, ModelContext, ModelHandle,
...
pub struct AppContext {
    ...
    /// that `riftui_core` does not depend on `reqwest`.
    #[allow(clippy::type_complexity)]
...
impl AppContext {
    ...
    /// Used to load fallback fonts without pulling `reqwest` into `riftui_core`.
    ...
    pub fn open_view_tree_debug_window(&mut self, target_window_id: WindowId) {
        ...
        let options = AddWindowOptions {
            ...
            window_instance: Some("dev.rift.riftui-debug".to_owned()),
