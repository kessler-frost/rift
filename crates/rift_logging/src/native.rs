...
fn sentry_log_filter(md: &log::Metadata) -> sentry_log::LogFilter {
    if rift_core::errors::should_ignore_log_for_sentry(md) {
    ...
    match md.target() {
        ...
        "riftui_core::core::redraw_frame" => sentry_log::LogFilter::Ignore,
        ...
        t if t.starts_with("rift::crash_reporting::") => sentry_log::LogFilter::Ignore,
...
fn init_internal(
    ...
    let integration_test = env::var("RIFT_INTEGRATION").is_ok();
...
fn init_log_directory() -> Result<std::path::PathBuf> {
    cfg_if::cfg_if! {
        if #[cfg(target_os = "macos")] {
            ...
            Ok(rift_core::paths::state_dir())
        } else if #[cfg(windows)] {
            Ok(rift_core::paths::state_dir().join(rift_core::paths::RIFT_LOGS_DIR))
        } else {
