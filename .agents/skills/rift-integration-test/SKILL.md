---
name: rift-integration-test
description: Writes, runs, and debugs Rift integration tests using the custom Builder/TestStep framework in `crates/integration`. Use when adding a new integration test, fixing a failing integration test, wiring a test into the manual runner or nextest suite, or verifying end-to-end UI and terminal behavior in Rift.
---

# Rift Integration Tests

Use this skill for Rust integration tests in Rift's custom framework under `crates/integration/`.

These are not ordinary unit tests. They boot a real Rift app instance, give it an isolated test home directory, drive it with synthetic UI and terminal events, and poll assertions until success or timeout.
...
- `crates/integration/tests/common/mod.rs`
  ...
  - Forwards a limited set of env vars (`PATH`, `RUST_*`, `RIFT_*`, `RIFTUI_*`, `WGPU_*`, display-related vars).
...
- `app/src/integration_testing/`
  - High-level helpers and assertions for common Rift behaviors.
...
This means integration tests should be written for a hermetic environment. Do not rely on the developer's real shell dotfiles, home directory contents, or persisted Rift settings.
        ...
        .with_step(
            new_step_with_default_assertions("Assert some UI state")
                .add_named_assertion("specific assertion name", |app, window_id| {
                    ...
                    riftui::integration::AssertionOutcome::Success
                }),
        )
}
```
...
Rift's wrapper automatically gives you:
...
- real-display support if `RIFTUI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS` is present
...
Use this to set persisted Rift preferences before the test starts.
...
```bash
RIFTUI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS=1 cargo run -p integration --bin integration -- test_name
```

Or with nextest:

```bash
RIFTUI_USE_REAL_DISPLAY_IN_INTEGRATION_TESTS=1 cargo nextest run --no-fail-fast --workspace test_name
```
...
```bash
RIFTUI_PAUSE_INTEGRATION_TEST_ON_FAILURE=1 cargo run -p integration --bin integration -- test_name
```
...
```bash
RIFTUI_PAUSE_INTEGRATION_TEST_AT_EVERY_STEP=1 cargo run -p integration --bin integration -- test_name
```

### Video and screenshots

If the task is specifically about recording a test, collecting screenshots, or validating overlay/video artifacts, also use the `integration-test-video` skill (located at `.rift/skills/integration-test-video/SKILL.md`).
