...
description: Write, improve, and run Rust unit tests in the rift Rust codebase.
---

# Rust Unit Tests in rift
...
- Prefer `riftui::App::test` for deterministic unit tests around views/models.
- Initialize app models once, then mutate via `update` and assert via `read`.

```rust
use riftui::App;
// In app crate tests prefer `crate::test_util::...`; from other crates use `rift::test_util::...`.
use rift::test_util::{terminal::initialize_app_for_terminal_view, add_window_with_terminal};
...
```rust
use rift::features::FeatureFlag; // or `use crate::features::FeatureFlag;` inside the app crate
